...
echo "· jar: $jar"
pgrep -f "Xvfb $D" >/dev/null || { Xvfb "$D" -screen 0 1400x900x24 >/dev/null 2>&1 & }
sleep 2 2>/dev/null || DISPLAY=$D xdotool sleep 2
pgrep -f "xfwm4" >/dev/null || { DISPLAY=$D xfwm4 --daemon >/dev/null 2>&1 & }
pkill -f "^java -jar" 2>/dev/null
rm -f "$HOME/.config/yxi/lock"
DISPLAY=$D nohup java -jar "$jar" > "$OUT/run.log" 2>&1 &
ok=0
...
tap $((wx+55))  $((wy+wh-21)); shot 2-配置
tap $((wx+193)) $((wy+wh-21)); shot 3-我的
