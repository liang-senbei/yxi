...
## 跟安卓版的功能对账（2026-08-28 · 第二轮）

安卓在 0.9.27。**下面这张表是当前真实状态，别当成愿望清单。**
...
| 底部四栏 会话/主机/**配置**/设置 | |
| 工作区四模式 终端/对话/**文件**/**实验室** | |
| **文件浏览** | 面包屑、上一级、跳转（含最近去过）、图片/markdown/html/json/代码分流、阅读↔源码 |
| **文件保存到手机** | 图片视频→相册（只申请「仅添加」权限），其余→Documents（「文件」App 可见）+ 系统分享 |
| **实验室** | 同一个 manifest；html(JS 开)/GIF(真的会动)/静态图 预览；保存原文件**走 SFTP 不是存预览那份**；左滑置顶/删除 |
| **配置页** | 抓取脚本跟安卓**逐字同一份**，密钥服务器侧打码；**可编辑**（json 先校验 + 改前 `cp` 备份两道保险） |
| **工单中心** | 同一个 `~/.yxi/tickets.jsonl` |
| **⚡模式快切 / 常用语 / 忙时「■ 停」** | |
| **选择器** | 问题正文、标签栏、上一题下一题、复核页提交；跟安卓**用同一批真机抓屏样本**测 |
| **盯屏推流** | 服务器主动推，轮询留作兜底 —— 「点一项等十秒」的根治 |
| **顶栏「▶ 正在做」+ TodoWrite 卡** | |
| **本对话的模型/思考强度/模式/上下文用量** | 样本抄自真转录；tokens 三项相加 |
| **看未提交的改动（git diff）** | + 绿 / − 红 / @@ 青 |
| **上次活动时间以转录 mtime 为准** | tmux/cc-state 都会陈旧（#130） |
| 版本号 | 0.9.27 (71)，跟安卓同步 |

**逻辑层测试 158 个，全过。** 关键样本（选择器抓屏、转录行、`/model` 回执）
跟安卓侧用的是同一批真实数据 —— 两端对同一份证据负责。

### ⚠️ 还没做

| 缺什么 | 情况 |
|---|---|
| **分享到会话** | 要单开一个 Share Extension target（安卓那边只是一个 intent-filter） |
| 桌面小组件 / 快捷设置磁贴 | iOS 有 WidgetKit 和 Control Widget，能做，只是还没做 |

### ❌ iOS 上本来就做不到 / 必须换做法

见 [`docs/与安卓版的差异.md`](docs/与安卓版的差异.md)。要点：
**从服务器直接装 App、App 自己装更新** 这两件在 iOS 上做不到；
**后台常驻 / 通知上直接批** 必须换做法。这不是没做，是苹果不允许。

### 怎么验的

没有 Mac 也能验：GitHub 的 macOS runner 编译 → 起 iPhone 16 模拟器 → 装上去 →
**在 runner 上现起一台真 sshd + tmux + 样本文件**，App 用**自己的密钥**连
`127.0.0.1`（模拟器跟 Mac 共用网络栈，等同安卓的 `10.0.2.2`）→ 逐页截图传回。
所以截图里的会话、文件、终端都是**真连上去**的，不是假数据。
