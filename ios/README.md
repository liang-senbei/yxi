...
## 跟安卓版的功能对账（2026-08-28 · 第三轮）

安卓在 0.9.27。**这张表是当前真实状态。**

⚠️ 前两版这张表**比真实状态乐观**：有四条把「解析器写好了」当成「功能做了」
（模型/模式显示、上次活动时间、保存进相册、json 折叠树），而 `⚠️ 还没做` 那栏
只列了两项、漏掉一大片。是一次独立对账把它揪出来的。**写这张表时按「用户点得到吗」判，
不是按「代码在不在」判。**

### ✅ 已对齐（界面真的有）

会话看板（等你/干活中/已完成/空闲、置顶、**静音**、**「多久没动了」**、**＋新建会话**、
回它一句失败会说话）· 悬浮卡片墙 · 主机管理（**长按看订阅额度**）· 终端（D-Pad、按键条）·
对话（**盯屏推流**、待答卡片与多选/上一题下一题、⚡模式快切、常用语、忙时「■停」、
**TodoWrite 卡 + 顶栏「正在做」**、**本对话的模型/思考强度/模式/上下文**、
**看 git diff**、**路径可点直接开文件**、**注入内容与 API 报错分开渲染**、
**草稿落盘 + 发失败把话还回来**、**斜杠命令提示条**、附件编号且点名字可预览）·
**文件浏览**（面包屑/跳转/多格式/阅读↔源码/保存到手机）· **实验室**（html·会动的 GIF·图片预览、
保存原文件走 SFTP、左滑置顶删除）· **配置页可编辑**（读不到不给改 + json 校验 + 备份没成功不写）·
工单中心 · **前台盯梢铃铛** · **每个会话记住上次看哪一面** · 更新检查先走公网下载页

**逻辑层测试 257 个全过。** 关键样本（选择器抓屏、转录行、`/model` 回执、`/usage` 输出）
跟安卓用的是同一批真实数据。
...
| 缺什么 | 情况 |
|---|---|
| 转录增量解析 | 现在每 300ms 重解整个 buffer；安卓有 `Incremental`（#86 实测 4MB 会卡） |
| 终端历史模式 | 滑动翻 Claude 自己的视图（送 SGR1006 滚轮） |
| 界面风格 3 档 + 中英切换 | 全硬编码中文 |
| 图标用的是 emoji | 安卓已全部换成矢量（不跟主题变色） |
| `Liveness` 写好没接线 | 切网后 TCP 半开时判不出假活（#81） |
| 实验室单条左滑删除 | 现在只能整栏删 |
| 控制中心按钮（iOS 18） | 写过一版编不过，撤了，等有真 Xcode 能编一次再加 |

### ❌ iOS 上做不到 / 必须换做法

见 [`docs/与安卓版的差异.md`](docs/与安卓版的差异.md)。要点：**从服务器直装 App、App 自己装更新**
做不到；**后台常驻盯梢、通知上直接批**必须换做法（现在是前台盯梢，界面文案如实写了）。

### 怎么验的

没有 Mac 也能验：GitHub 的 macOS runner 编译 → 起 iPhone 16 模拟器 → 装上去 →
**在 runner 上现起一台自己的 sshd + tmux + 样本文件**，App 用**自己的密钥**连
`127.0.0.1:2222`（模拟器与 Mac 共用网络栈，等同安卓的 `10.0.2.2`）→ 逐页截图传回。

**已经证实真的连上了**：日志里 `SSH_OK`、`tmux ls` 列出 `cc-demo`、App Group 容器拿到；
截图里文件页列出真实 SFTP 目录、终端接上真 tmux、会话卡显示「9 分钟前」。

⚠️ **截图这一步抓到过两个编译和单测都发现不了的问题**：四颗按钮被压成竖排单字、
实验室/配置页在连上之前打开就永远显示「没连上」。这就是它的价值。

本地还有两道秒级检查（都用注入错误自验过）：
`dev/precheck.sh`（文本体检）、`dev/projcheck.sh`（在临时目录生成工程核对结构）。
