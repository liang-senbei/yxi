...
## 跟安卓版的功能对账（2026-08-28）

安卓在 0.9.27。这份 iOS 移植原本停在 8/23，这次把**逻辑层全部追平**、
界面补了四块。**下面这张表是当前真实状态，别当成愿望清单。**

### ✅ 已经对齐（模拟器里跑过、截图确认）

| | 说明 |
|---|---|
| 底部四栏 会话/主机/**配置**/设置 | 原来只有三栏 |
| 工作区四模式 终端/对话/文件/**实验室** | 原来只有三种 |
| **配置页** | 分工具分类目浏览，抓取脚本跟安卓**逐字同一份**，密钥服务器侧打码 |
| **实验室** | 读同一个 `~/.yxi/lab/manifest.json`，按类型分栏、标「由谁生成+北京时间」、勾选审核 |
| **工单中心** | 写进同一个 `~/.yxi/tickets.jsonl`，自动带版本号+机型 |
| **⚡模式快切** | 默认命令是安卓侧从真转录核过的（`/model claude-opus-5[1m]` 等） |
| **常用语 chips** | 没打字时露出来 |
| **忙时「■ 停」** | 送 Esc 打断 |
| **选择器：问题正文/标签栏/上一题下一题/复核页提交** | 解析器四处修复 + 界面；跟安卓**用同一批真机抓屏样本**测 |
...
| 缺什么 | 情况 |
|---|---|
| **文件浏览界面** | SFTP 那层早就好了且有测试，缺的是界面 |
| **实验室的预览** | 现在只列条目；安卓能看图/GIF/网页、左滑置顶删除、保存原图到相册 |
| **配置页的编辑** | 现在只读。安卓能改（改前自动备份 + json 先校验）—— 这两道保险没做好之前不开写 |
| TodoWrite 清单卡 / 看 git diff / 顶栏模型+模式 | 都没做 |
| 保存文件到本地 / 分享到会话 | 没做 |
| 盯屏推流（watchScreen） | 命令builder 和测试都有了，界面还在用轮询 |
