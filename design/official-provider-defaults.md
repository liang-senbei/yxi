...
| Gemini | Gemini Code Assist 企业订阅 | 2026-05-19官方迁移公告明确：2026-06-18起个人免费、Pro/Ultra不再由Gemini CLI服务，改用Antigravity CLI；Standard/Enterprise和付费API仍支持。企业订阅与API分别列项，不自动替换运行器。[官方公告](https://developers.googleblog.com/an-important-update-transitioning-gemini-cli-to-antigravity-cli/) |
...
当前实现：`OfficialProviderProfiles`定义各运行器的内置项和首选，服务器配置页固定显示相应卡片及官方说明，Grok启动命令按官方资料更新为`grok`（不因此开放尚未完成的创建适配）。卡片明确显示账号/线路尚未核对，不宣称已启用。Codex本地官方进程校验已实现；其余登录状态读取、登录UI和真实线路应用仍待逐运行器完成。本文件不把静态卡片当作默认官方发送已验收。
