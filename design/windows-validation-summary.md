...
当前代码批次b6757ea交cc-yxi集中编译主源与测试源，范围为线路影响范围、答复草稿保留及线路配置后的新建入口；不重跑其他Agent专项。8035b87候选保持不变。

UI Agent说明双任务隔离场景未真正建立：假运行器按进程计数且每次启动都返回相同threadId，导致同键登记被替换。该用例标未验证，不据此认定真实运行器存在同ID问题。当前最后一遍结束后只收报告，不继续校准环境。

## 上线后第一批集成结果（2026-09-19）

- cc-yxi在a454224集中编译主源和测试源通过（7秒、0错误）；范围包括滚动修复、恢复/新建测试合入、普通附件、答复退出保护及运行器模型报告。后续阅读位置/目标条/历史结果改动不在此基线内。
- pilot普通附件定向15/15通过，测试提交92f36f2后重放为b6ba557，已合入。覆盖仅文件/图文混合的真实适配器序列化、相对路径入队拒绝、控制字符路径投递前拒绝且不发turn/start。
- 无真实模型调用，UI提示呈现与Windows实机未包含在上述证据中，线上1.3.0未更换。
...
- cc-yxi在a79adb8完成主源/测试源集中编译，BUILD SUCCESSFUL（4秒、0错误）；覆盖信箱筛选/保留期、钱包订单和自动续费，未发布。
- 合入logto的80425c0兑换解析检查（原基线b2dc537，7项通过，无网络/真实兑换）。其中空JSON成功断言已按4a17600修复改为拒绝；调整后的该类尚待Agent定向运行，不沿用旧7项结果宣称新版本通过。

## 商城与通知检查整合

- 合入logto的7aa2c10（本地89055e2）：商城4项假transport/临时文件检查通过，覆盖分单位、请求身份先落盘、同owner未确认禁新ID、重启沿用及结果确认后清理；无真实交易。
- 合入pilot的3b795c8（本地3cd6d7e）：实时通知、重复完成去重、历史不补发及置顶/静音过滤检查代码。检查结果不等于Windows托盘点击全链路验收。
- 设置页新增项目增多后加上滚动区域，历史保留独立分组，避免低高度窗口遗漏设置。界面调整未实机验证。

## 历史保留与转录缓存编译结果

- pilot的InstructionQueueTest 8/8通过（4既有+4新增），37e9f61已合入：终态可清、Unknown/进行中/无时间旧数据不清，清理后同ID摘要去重、重载标记及常规备份不保留旧正文。仅临时文件，未清理用户数据；损坏副本与清理交互未覆盖。
- cc-yxi在9dfa648集中编译主源和测试源成功（17秒、0错误），覆盖历史保留、转录缓存基础与ChatPane接线。缓存3项定向检查仍在进行；后续2af2c04系统语音入口不在该编译基线内。

## 缓存、收藏与离线ASR阶段结果

- 合入pilot的ca10cfa缓存测试：3项进程内检查通过，覆盖旧读者撤销、解析状态与偏移整体提交、再次获取后增量追加。没有覆盖真实SSH流/GUI切换。
- cc-yxi报告9caec0f及59ffb3e主源/测试源编译成功，分别覆盖系统语音/收藏入口和本机whisper.cpp适配。此前报告误称ca10cfa已在9caec0f祖先链，实际测试在本次才合入；不沿用该误述。
- 收藏入口已由entertainment静态核对主机绑定、离线禁用、先刷新再打开、表单预填及单次写入迁移；未做真实创建/界面交互。离线ASR假CLI检查仍在进行，未使用真实麦克风或模型。
