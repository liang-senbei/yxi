...
| 对话界面精确回退续聊 | 开发线已接入一键入口。真实 Claude 2.1.278 + SSH 隔离测试通过普通文字连续两次回退、重启后核验、核验前断线恢复、不重复请求，以及实际 Read 结果和增量/冷加载消息分支。恢复身份在模型请求前持久化。共用 Compose 编辑弹窗已验证渲染与 Esc 取消；Windows 整个界面到运行器的联动、首轮、附件、Codex 和更早中断阶段的自动恢复仍未完成。开发改动未发布；详见 windows-rewind-verification.md。 |
