...
官网为1.4.10，冻结4bf9712，CI35620405512成功。1.4.9冻结7e9bb18，CI35612716002成功。详细产物及校验见对应release文档，不以开发分支版本号推断线上内容。
...
| 清爽配置/头像/插件分类 | 新服务器+运行器+供应商首页、头像全图标、本地/服务器插件分类、移除底部配置/我的行已在开发线；尚未发布。 |
| 多运行器供应商配置 | Claude/Codex已实现；Gemini适配正在独立开发，OpenCode/Grok Build/Hermes尚未接入真实应用逻辑。不能以选择chip当完成。代理UA/header/body等用户明确功能仍未完成。 |
| 对话界面精确回退续聊 | 开发线保留真实sourceUuid与可编辑消息；精确adapter仍待合入，原生选择器仍需手动选轮次。被拒的绕过权限实现未进入主线。 |
| 一键Android模拟器 | 开发线有环境发现、进程启动停止、日志、硬件加速检查、指定设备安装APK/启动包、官方工具包下载界面。SDK组件/镜像安装、AVD创建、项目构建运行、内嵌画面与Agent工具桥接仍未完成。 |
| 网页单击侧栏/Ctrl外部 | 1.4.9发布链接路由；中文标点及www裸URL修复在开发线，尚未发布。 |
