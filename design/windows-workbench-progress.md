...
首版 `d1a7797` 编译 `run.iLTe58`，真实 Compose 界面测试 `run.VVnrjh` 通过。复核提交 `7fa9029` 编译 `run.19d3tJ`；界面测试 `run.Npuhpz` 1项、SSH目录URL保存/读取 `run.3Gl7nN` 1项、1M声明处理 `run.vEtiXG` 2项、配置编辑 `run.yobCw9` 5项，共9项通过，失败/错误/跳过均为0。追加长ID悬停显示与完整值选择 `ae82402` 编译 `run.VkDXHi`、界面测试 `run.9AEYeF` 通过；长ID浮层、宽窄/深色及实际供应商编辑页截图已人工查看。实际深色截图另发现页标题未继承正文色，已在供应商编辑页限定内容色，下一轮构建复核。
