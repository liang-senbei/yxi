...
- W09仍未完成：真实API密钥/模型请求验证、提供方能力与模型列表、CLI实际生效/重开会话闭环、线路删除/导入导出/备用策略、原子目录清单写入与失败回滚、双主机矩阵。现有共享saveList仍采用SFTP直接写回，前端比较无法替代原子并发控制，发版前需加固。Android完整构建回归也仍待SDK环境验证。
