...
版本：0.2 · 2026-09-23 · 已开始实施；L1部分功能已验证，整体仍未交付。
...
开发分支已加入顶级“本地”位置、独立本地配置视图、安装与版本检测、可执行文件选择、项目目录登记和Codex原生历史浏览。npm清单解析为原生exe或固定Node入口参数，不能把`.cmd/.ps1`直接传入进程。Codex多版本分开展示；读取账号状态不强制刷新登录，列表包含各来源/供应商及归档，历史优先分页，旧接口回退有8MB响应上限。

2026-09-23已用产品实际编译类在用户Windows上只读核对：PATH Codex 0.155.0-alpha.16、npm Codex 0.149.0、桌面附带Codex 0.155.0-alpha.16、npm Claude Code 2.1.228被正确识别；三种Codex入口均返回23条未归档历史，三个截图标题各匹配1条，并读取20轮历史分页；全局Codex配置哈希不变，未输出正文、复制凭据或恢复会话。旧npm版本有效provider读取无法确认，界面保留“无法确认”，不以ChatGPT Pro登录标签猜测其实际线路。

当前对话浏览是只读，不提供原会话发送或接管。界面中的官方订阅仅为原生账号状态展示；L2的“新建默认官方订阅”、独立线路和接管互斥尚未接入。旧本地任务记录保留入口，其执行仍按原有外部配置；不得把这条旧路径算作官方默认已完成。

尚缺：WSL枚举/探测、未定位包装器的细化原因、安装引导、Claude/OpenCode/Hermes原生会话适配、完整会话工具/附件渲染、原生项目分组同步、Windows整页GUI/DPI/IME验证及完整L2–L5。先前目标均保留，完成范围与证据持续记入 [实施记录](windows-workbench-progress.md)。
