...
class CodexMcpHttpHeadersNativeTest {
    @Test fun `native HTTP MCP sends the complete environment header value without persisting it`() = runBlocking {
        ...
        try {
            ...
            val config = File(home, "config.toml").apply { writeText("model = \"fixture-model\"\n") }
            ...
            val runtime = LocalRuntimeInstallation("codex", "isolated", listOf("/opt/native/claude"), home.path, "0.153.4")
            val arguments = SharedMcpSettings.codexArguments(records, "@local") + "app-server"
            assertTrue(arguments.none { "fixture-http-token" in it })
            val transport = LocalCodexTransport.start(runtime, arguments,
                System.getenv() + ("YXI_TEST_MCP_AUTH" to "Bearer fixture-http-token"))
            CodexAppServer(transport).use { client ->
                ...
                LocalCodexProfiles.verifySharedMcp(effective, records)
                assertFalse(effective.toString().contains("fixture-http-token"))
                client.startThread("/sandbox/home")
