...
fun App(state: AppState) {
    val emulatorOpen = remember { mutableStateOf(false) }
    AndroidEmulatorPanel(emulatorOpen.value) { emulatorOpen.value = false }
                            ...
                            Row(Modifier.fillMaxWidth().height(48.dp).padding(horizontal = 12.dp), verticalAlignment = Alignment.CenterVertically) {
                                ...
                                Box {
                                    ...
                                    DropdownMenu(taskMenu.value, { taskMenu.value = false }) {
                                        ...
                                        DropdownMenuItem(text = { Text("Android 模拟器") }, onClick = { taskMenu.value = false; emulatorOpen.value = true })
                                    ...
                                    Row(Modifier.fillMaxSize()) {
                                        if (!panel || !compact) Column(Modifier.weight(1f).fillMaxHeight()) {
                                            if (replaced) Text("原会话已结束，同名会话是新任务。请在左侧重新选择；旧草稿已保留。", Modifier.padding(24.dp))
                                                ...
                                                0 -> ChatPane(conn, sess, state.instructions, savedDraft = state.chatDrafts.getOrPut(taskKey) { mutableStateOf(androidx.compose.ui.text.input.TextFieldValue()) }, displayName = state.navigation.title(taskKey), onRoutes = { state.openRoutes() }, onTerminal = { state.tab = 1 }, modelSwitches = state.modelSwitches)
