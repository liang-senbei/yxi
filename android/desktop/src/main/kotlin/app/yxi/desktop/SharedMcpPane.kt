            ...
            Column(Modifier.heightIn(max = 520.dp).verticalScroll(rememberScrollState()), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                ...
                if (advanced) {
                    if (remote) {
                        ...
                        Text("变量的值应包含完整请求头，例如 Bearer 加令牌。此处只填写变量名。OpenCode 的请求头引用尚未接入。", style = MaterialTheme.typography.bodySmall)
            ...
            OutlinedCard(Modifier.fillMaxWidth()) { Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                if (!record.retired) {
                    Row {
                        ...
                        } }, enabled = registry.problem.isBlank() && record.definition.headerVariables.isEmpty(), label = { Text("OpenCode 新会话") })
                    ...
                    if (record.definition.headerVariables.isNotEmpty()) Text("此配置含请求头引用，OpenCode 暂不可选。", style = MaterialTheme.typography.bodySmall, color = t.textMuted)
