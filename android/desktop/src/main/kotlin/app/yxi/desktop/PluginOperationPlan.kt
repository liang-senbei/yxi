...
internal object PluginOperationPlan {
    fun command(request: JSONObject): String {
        require(request.optString("action") in setOf("prepare", "set", "status", "restore", "prepare-install", "install", "uninstall", "update"))
    ...
    fun result(raw: String): JSONObject {
        ...
        return JSONObject(value.removePrefix("__YXI_PLUGIN_OP__:")).also {
            require(it.getString("state") in setOf("unsupported-config-home", "invalid", "busy", "missing", "unknown", "invalid-directory", "unsupported-linked-config", "operation-conflict", "prepared", "changed", "configured", "unconfirmed", "restored", "restore-unavailable", "installed", "catalog-changed", "already-installed", "uninstalled", "updated"))
        }
    }
}
