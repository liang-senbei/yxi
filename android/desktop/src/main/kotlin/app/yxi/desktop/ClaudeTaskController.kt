...
internal class ClaudeTaskController(val taskKey: String, private val client: ClaudeControlClient, private val queue: InstructionQueue,
    ...
    suspend fun sendNext(): Unit = withContext(Dispatchers.Swing) { mutation.withLock {
        check(ready && !disposed && !busy && pendingApprovals.isEmpty()) { "Claude 当前不能接收新指令" }
        ...
        try {
            ...
            val outcome = if (result.opt("is_error") == false && result.optString("subtype") == "success") RuntimeTurnState.Completed else RuntimeTurnState.Failed
            ...
            note = if (outcome == RuntimeTurnState.Completed) "本轮处理结束" else "本轮未成功，请核对结果"
        } catch (e: Exception) {
            ready = false; client.close()
            note = if (started == null) "发送前检查未通过，指令仍在本地：${e.message.orEmpty()}"
