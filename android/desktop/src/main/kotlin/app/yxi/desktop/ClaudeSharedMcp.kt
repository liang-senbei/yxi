...
internal object ClaudeSharedMcp {
    internal fun reportsMissing(name: String, text: String): Boolean = text.lineSequence().any {
        val line = it.trim().removePrefix("Error: ")
        line == "No MCP server found with name: $name" || line.startsWith("No MCP server named \"$name\".")
    }
    ...
    suspend fun prepare(binary: File, directory: File, definitions: List<SharedMcpDefinition>, destination: File, cancelled: () -> Boolean = { false }): List<String> = withContext(Dispatchers.IO) {
        ...
        definitions.forEach { definition ->
            ...
            try {
                ...
                check(reportsMissing(definition.name, text)) {
