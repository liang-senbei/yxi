...
internal object NativeFirstTurnKeys {
    ...
    fun command(identity: RewindLiveVerification.RuntimeIdentity, source: RewindTarget, screen: String,
        ...
        val packet = JSONObject().put("session", identity.sessionName).put("runtime", identity.runtimeId)
            ...
            .put("file", source.file).put("size", source.size).put("mtime", source.modifiedNs)
...
try: st=os.stat(p['file'])
except OSError: reject('history')
if st.st_size!=p['size'] or st.st_mtime_ns!=int(p['mtime']): reject('history')
