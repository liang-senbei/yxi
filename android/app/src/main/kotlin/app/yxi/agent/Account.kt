...
object Account {
    ...
    // ⚠️ **模型和 HTTP 都搬进 :core 的 [AccountApi] 了**(老板 2026-09-08 派桌面版,手机和桌面共用一份)。
    //    这里只留 Android 那一半:登录态、加密存储、浏览器跳转、Compose 状态。
    //    下面这些别名是为了不动界面层的几十处引用 —— `Account.Me` 之类照旧能写。
    private val Api = AccountApi


    private const val AUTH = AccountApi.AUTH
    private const val API = AccountApi.API
    private const val APP_ID = AccountApi.APP_ID
    const val REDIRECT = "io.yxi.app://callback"
    private const val SCOPES = AccountApi.SCOPES
    ...
    internal fun patchMe(f: (AccountApi.Me) -> AccountApi.Me) { me?.let { me = f(it) } }
    ...
    fun load(ctx: Context) {
        ...
        me = p(ctx).getString("auth.me", null)?.let { runCatching { AccountApi.parseMe(JSONObject(it)) }.getOrNull() }
    }
    ...
    fun startLogin(ctx: Context): Boolean {
        val verifier = AccountApi.randomUrlSafe(64)
        val state = AccountApi.randomUrlSafe(16)
        p(ctx).edit().putString("auth.verifier", verifier).putString("auth.state", state).apply()
        val challenge = AccountApi.b64url(MessageDigest.getInstance("SHA-256").digest(verifier.toByteArray()))
    ...
    fun signOut(ctx: Context) {
        ...
        if (rt != null) Thread {
            ...
            runCatching { AccountApi.form("$AUTH/oidc/token/revocation", mapOf("token" to rt, "client_id" to APP_ID)) }
        }.start()
        signedOutWhy = null
    ...
    suspend fun refresh(ctx: Context): String? = withContext(Dispatchers.IO) {
        ...
        val (c, body) = AccountApi.req("$API/api/me", "GET", tk, null)
        if (c !in 200..299) {
            android.util.Log.w("YxiAccount", "/api/me HTTP $c: " + body.take(200))
            return@withContext AccountApi.httpErr(c, body, Tz::date)
        }
        runCatching {
            val o = JSONObject(body)
            me = AccountApi.parseMe(o)
    ...
    suspend fun saveProfile(ctx: Context, nickname: String?, signature: String?, avatar: String? = null): String? =
        withContext(Dispatchers.IO) {
            ...
            val (c, resp) = AccountApi.req("$API/api/me/profile", "PATCH", tk, body.toString())
            when {
                c in 200..299 -> {
                    runCatching { me = AccountApi.parseMe(JSONObject(resp)); p(ctx).edit().putString("auth.me", resp).apply() }
                    null
                }
                else -> AccountApi.httpErr(c, resp, Tz::date)
            }
        }


    suspend fun redeem(ctx: Context, code: String): Result<AccountApi.Redeemed> = withContext(Dispatchers.IO) {
        val tk = token(ctx) ?: return@withContext Result.failure(Exception("没登录"))
        val (c, resp) = AccountApi.req("$API/api/me/redeem", "POST", tk, JSONObject().put("code", code).toString())
        if (c !in 200..299) return@withContext Result.failure(Exception(AccountApi.httpErr(c, resp, Tz::date)))
        ...
        val msg = when (o.optString("kind")) {
            "balance" -> {
                val got = AccountApi.yuan(o.optLong("amountCents"))
                val now = AccountApi.yuan(o.optLong("balanceCents"))
        ...
        Result.success(
            AccountApi.Redeemed(
                ...
                tier = when (o.optString("tier")) { "ultra" -> AccountApi.Tier.Ultra; "pro" -> AccountApi.Tier.Pro; else -> AccountApi.Tier.Free },
     ...
     */
    suspend fun claimMail(ctx: Context, id: String): AccountApi.Claim? = withContext(Dispatchers.IO) {
        val tk = token(ctx) ?: return@withContext null
        val (c, body) = AccountApi.req("$API/api/mail/$id/claim", "POST", tk, "{}")
        if (c !in 200..299) return@withContext null
        runCatching {
            val o = JSONObject(body)
            val r = AccountApi.Claim(
    ...
    suspend fun mail(ctx: Context, before: String? = null): Pair<List<AccountApi.Mail>, String?>? =
        withContext(Dispatchers.IO) {
            val tk = token(ctx) ?: return@withContext null
            val url = "$API/api/mail?limit=30" + (before?.let { "&before=$it" } ?: "")
            val (c, body) = AccountApi.req(url, "GET", tk, null)
            if (c !in 200..299) return@withContext null
            runCatching {
                val o = JSONObject(body)
                            ...
                            attachments = it.optJSONArray("attachments").let { a ->
                                (0 until (a?.length() ?: 0)).mapNotNull { j ->
                                    a?.optJSONObject(j)?.let { x ->
                                        AccountApi.Attach(x.str("kind"), x.optLong("amount"), x.str("name"), x.str("code"))
                                    }
                                }
                            },
    ...
    suspend fun markMailRead(ctx: Context, id: String): Boolean = withContext(Dispatchers.IO) {
        ...
        val (c, body) = AccountApi.req("$API/api/mail/$id/read", "POST", tk, "")
    ...
    suspend fun apiGet(ctx: Context, path: String): JSONObject? = withContext(Dispatchers.IO) {
        val tk = token(ctx) ?: return@withContext null
        val (c, body) = AccountApi.req("$API$path", "GET", tk, null)
        if (c !in 200..299) return@withContext null
