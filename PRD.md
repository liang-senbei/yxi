...
| P0-13 | **⭐ 用量显示** | 5 小时额度 + 本周额度 + 成本，**按服务器分**（`ccusage` 读那台机的 `~/.claude`，天然独立）。主机列表紧凑条 + 会话看板详情卡。**附录 K** |
| P0-14 | **一键装公钥** | 用密码连上一次 → 一键把 App 公钥 append 进 `~/.ssh/authorized_keys` → 之后免密。相当于 `ssh-copy-id`（§2.4） |
...
| 13 | **用量** | 主页 | ✅ **做** | **P0** | `cc-quota` 已有现成的，包一层就行 |
...
> 批注原文（文件模式，83%, 2.8% ——正落在标题栏）：「文件模式可以支持方便导航到对应路径的工作文件夹吗」

**入口 = 点面包屑**（就在你批注的位置），下拉一个跳转面板：

| 项 | 说明 |
|---|---|
| **直接输入路径** | 一个输入框，支持 `~` 展开 |
| **会话所在** | 该会话的 cwd——默认起点，随时一键回去 |
| **最近去过的** | 本机最近访问的几个目录（App 侧记，不用服务器帮忙） |
| **收藏** | 手动固定的目录 |
| **`~` / 附件暂存区** | 常用位置。附件暂存区 = `/root/src/tmp/<项目>/`（附录 F） |

> 对应 Moshi 的「跳转到…」和「最近的目录」（§5.5 第 10、11 条），
> 但**我们的最近目录记在 App 侧**——不用 `moshi-hook cwd-list` 那套服务器历史库。
...
| 项 | 值 |
|---|---|
| 圆角 | 卡片 **28px** · 内嵌块 22px · **控件一律药丸 100px** · 小标签 14~16px |
| 间距 | 页面留白 **18px** · 块间距 **22px** |
| 字号 | 正文 **15px** · 标签 12.5px · 标题 17px · 等宽 13px |
| 字体 | Space Grotesk（界面）+ JetBrains Mono（代码/路径/时间）——沿用 |
| 强调色 | 铜 `#ffb787`（主操作）· 青 `#8fd8c6`（干活中/Edit）· **琥珀 `#ffc46b`（等你，全 app 只在需要你动手时出现）** |
| 触摸目标 | ≥44px（控件普遍抬到 48/52px） |

> ⚠️ **接受的代价**：同屏信息量比原方案（方向 A）少约**三分之一**。
> 对一个「扫一眼看哪个会话在等你」的工具这是真代价——所以**会话看板保留紧凑的列表视图**，
> 悬浮排列（J.3）作为另一种视图，而不是替换掉列表。

### J.2 ⭐ 滑动切卡的**视差过渡**（D17）

滑动切换会话卡片时，**三层以不同速度移动**，产生纵深：

| 层 | 速度 | 效果 |
|---|---|---|
| **焦点卡** | 1.0x | 跟手 |
| **左右邻居卡** | 0.86x + 缩放 0.86 + 不透明度 0.55 | 落后于手指，显得更远 |
| **背景** | 0.3x | 几乎不动 |

**实现**：`ViewPager2` + 自定义 `PageTransformer`——按 `position` 同时插值
`translationX`（乘以速度系数）、`scaleX/Y`、`alpha`。这是 Android 标准做法，不需要额外依赖。

> ⚠️ **必须尊重系统的「移除动画」设置**（无障碍）：开了就退化成直接切换，不做视差。
> 这不是可选项——对前庭障碍用户，视差会引发不适。

### J.3 ⭐ 悬浮排列的会话切换（D18）

用户：「切换 tmux 可以选择悬浮排列，就像手机的后台一样一个个排列起来」。

**你的 tmux 会话本来就像一堆运行中的应用**——这个隐喻是对的。

| 项 | 设计 |
|---|---|
| 形态 | 卡片轮播：中间焦点卡放大，左右邻居缩到 0.86 并压暗（视差就发生在这，见 J.2） |
| 卡片内容 | **`tmux capture-pane` 的实时缩略预览** ——一眼看出每个会话在干嘛，不用点进去 |
| 状态 | 左上角圆点：青=干活中 · **琥珀=等你** · 灰=已完成 |
| 左右滑 | 切换会话 |
| 点击 | 进入该会话 |
| **上滑** | ⚠️ **从看板归档，不杀 tmux 会话** |
| 杀会话 | **长按 + 二次确认**。杀 tmux 会话是**不可逆**的，绝不能是一个滑动手势 |

**与列表视图并存**：顶部 `[列表 │ 悬浮]` 切换，记住偏好。
- **列表**：信息密度高，适合「快速扫一遍谁在等我」
- **悬浮**：有预览，适合「我想看看那个会话现在什么样」

> ⚠️ **上滑不杀会话** 这条是硬约束。安卓后台的上滑就是关闭应用，用户会有肌肉记忆——
> 正因为如此才**更**不能让它杀会话：手滑一下就丢掉一个跑了两小时的构建，代价太高。

---

## 附录 K · 用量显示（D19）

> 用户：「能不能看 Claude Code 的用量……如果是订阅模式就显示 5 小时额度和周限……
> 我们可能会有好几个服务器，点开对应的服务器才是那个服务器的 tmux，
> 我觉得这个订阅模式的用量就跟对应的服务器关联吧」——**放置位置用户自己定了，是对的。**

### K.1 数据源：**本地转录就够，不用调任何 API**

已在本机验证：**转录里每条 assistant 消息都自带完整 `usage`**

```json
{"input_tokens":2, "cache_creation_input_tokens":12743, "cache_read_input_tokens":22349,
 "output_tokens":64000, "output_tokens_details":{"thinking_tokens":64000},
 "service_tier":"standard"}          // 同条消息还带 "model":"claude-opus-5"
```

→ **有 token 数 + 模型名，成本本地就能算。**

而且**现成的轮子已经在你机器上跑了**：`remote-dev-station/bin/cc-quota` 用的是
[`ccusage`](https://www.npmjs.com/package/ccusage)——读 `~/.claude/projects` 算 Claude 的
**5 小时滚动窗口**，`ccusage blocks --active --json` 输出：

| 字段 | 含义 |
|---|---|
| `startTime` / `endTime` | 当前 5 小时窗口 |
| `tokenCounts` / `totalTokens` | 出入 token / 含缓存总量 |
| **`costUSD`** | 成本（按模型定价算的） |
| `burnRate.{tokensPerMinute, costPerHour}` | 燃烧率 |
| `projection.remainingMinutes` | 距重置 |

**订阅模式和 API 模式都覆盖**：`costUSD` 是从 token × 模型定价算的，跟计费方式无关。

### K.2 ⭐ 它天然就是「按服务器分」的

`ccusage` 读的是**那台机器的 `~/.claude`**。所以：

> **每台服务器的用量本来就是独立的，不需要我们做任何关联工作。**
> 用户想要的「跟对应服务器关联」是免费的。
...
| 位置 | 显示什么 |
|---|---|
| **主机列表**（每行右侧） | 一条细进度条 + `$12.8`——**扫一眼看哪台烧得快** |
| **进入某台机后的会话看板顶部** | 完整卡片：5 小时额度进度 + 距重置 + `18.4M tok` / `$12.80` / `$4.1/h`，下面一条本周额度 |

**颜色沿用语义**：5 小时额度用**琥珀**（跟「等你」同族——都是「该注意了」），本周用青色。

### K.4 API / 中转站模式（NewAPI 之类）

用户提到「NewAPI 应该可以计费」。你的站长机 `~/.bashrc` 里那个第三方中转
（`cpa4.agentapi.best`）就是这类。

- **成本**：`ccusage` 已经算了，**不需要问中转站**
- **余额**：要问中转站自己的接口（NewAPI 系一般是 `/api/user/self`），**需要 relay 的 token**
  → **记为 P2，可选配置**。没配就只显示 ccusage 算出来的花费，不显示余额

> ⚠️ **不把中转站的 token 存进 App**。要显示余额的话，让 `yxi` 脚本在**服务器上**去问
> （token 本来就在那台机的环境里），App 只收结果。

### K.5 P0 范围

- ✅ 5 小时窗口（进度 / 距重置 / token / 成本 / 燃烧率）——`ccusage blocks --active --json`
- ✅ 本周——`ccusage weekly --json`
- ✅ 主机列表的紧凑条
- ⬜ P2：中转站余额（要额外配置）
- ⚠️ **`ccusage` 是外部 npm 包**：探测不到就把用量区隐藏 + 给一行安装提示，**不要显示假数据**
