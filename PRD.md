...
**安卓能侧载 APK，iOS 不能。** iOS 要装自签 App 只有三条路：99 美元开发者账号上架 /
TestFlight（也要账号）/ 免费证书自签但**每 7 天重签一次**。

**决定：先做安卓 APK，iOS 继续用原版 Moshi。** 等安卓这套跑顺再决定值不值得折腾。

> 好消息：**`yxi-agent` 是共用的**，而且它不依赖任何客户端形态（就是个读 stdin 写 stdout 的进程）。
> 将来给 iOS 做任何东西，服务器侧一行不用改。**现在做的不浪费。**
...
- ✗ 多用户、多租户、别人也能用的 SaaS
- ✗ **上架应用商店**（只侧载）
- ✗ **iOS 版**（第一期不做，见 §2.3；iOS 继续用原版 Moshi）
...
| 分发 | 微信 / `lapput` 传手机侧载 | 你已经在用这条路（Moshi 的 APK 就是这么来的） |
