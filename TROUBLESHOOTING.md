...
⚠️ **再往上一层：这一族的名字是「拿一个信号去推另一件事，而那个信号根本不携带这个信息」**
（cc-Yxi_Entertainment 2026-09-08 归纳的，两天里我们撞了三次，每次都以为是独立的失误）：
- 本条：「App 里**有**一台主机」→ 推「**有人想**跑上传压测」。主机是别人测试环境的残留，跟意图无关。
- **#321**：「谁做了 `:core` 拆分」→ 推「**现在**谁在改那个脚本」。做过相关改动 ≠ 此刻正在动这个文件。
- 拿号：脚本报「下一个空号 #318」→ 推「**没人要** #318」。脚本只看得见**落了盘的**，
  别人已经领了但还没提交的号它一无所知。
共同点是**信号是真的，推论是编的**：那个信号只回答了它自己那个问题。
识别办法：把推论写成「A 所以 B」，然后问一句「**A 为真而 B 为假**，可能吗？」——
可能，就说明这不是判据，只是相关。要么去拿真正回答 B 的那个信号（问一句、`peek` 一下、看 diff），
要么把机制改成让 B 直接可见（#317 那条就是这么办的）。
...
## #321 共用工作树里 `git checkout <文件>` 会抹掉别人**正在改**的未提交内容（cc-Yxi_pilot，2026-09-08）

**症状**：我改坏了 `dev/i18n-check.sh`，顺手 `git checkout dev/i18n-check.sh` 回滚 ——
文件确实回到了「干净」状态，但那是**上一次提交**的样子。
cc-Yxi_Entertainment 那会儿正在同一个工作树里改这个文件（拆 `:core` 之后 En.kt 搬走，扫描根要改成两个模块），**还没提交**。
（⚠️ 我第一版把受害者写成了 cc-Yxi —— 因为我从「谁做了 `:core` 拆分」推断「谁在改这个脚本」。
**别从「谁做了相关的改动」推断「现在谁在动这个文件」**，问一句或者 `yxi-hub peek` 一下就知道了。）
⚠️ 他那份**已经改完并跑通过**（输出「源码里 1252 句」），所以不是「没保存」——
是**落了盘的内容被覆盖**。工作区里的东西没有任何版本保护，这一点跟「没保存」完全不同。
他那份改动一起没了，而且没进过暂存区，`git fsck` 也找不回来。

**根因**：`git checkout <路径>` 的语义是「**用索引里的版本覆盖工作区**」——
它不区分「工作区里的改动是谁做的」。而这个仓库是**四个 agent 共用一个工作树**：
任何时刻别人都可能有未提交的活儿躺在里面。
更糟的是它**不给任何提示**：没有确认、没有 diff、只回一句 `Updated 1 path from the index`，
被覆盖的内容从此不存在于任何地方。

**修法 / 怎么避开**：
- **先看再动**：`git diff <文件>` 看一眼要丢的是什么。**diff 里出现你没写过的东西 = 停手**，
  那是别人的活儿（这次的信号很明显：头部注释写着我没写过的 `:core` 拆分说明，我却当成了自己弄坏的）。
- 想撤销的**只是自己刚才那几下**：用编辑器的撤销、或者先 `git stash push <文件>`（**stash 存得住，回滚存不住**），
  别用 `checkout` 这种「回到某个版本」的操作 —— 你要的是「撤销我的改动」，不是「回到上一次提交」。
- 真要回滚共用文件，先 `yxi-hub peek` / 问一句，或者干脆只回滚**自己新建的文件**。
- **改共用文件用整篇重写，别打补丁**（cc-Yxi_Entertainment 实测补的）：他第二次去改时用的是打补丁，
  而补丁锚点在「他的版本」和「被我回滚后的版本」里**都存在** —— 于是补丁**静默地打在了回滚后的内容上**，
  结果是「改完又变回去了一半」，还不报错。整篇重写会当场暴露内容不对。
- **提交时限定路径**（`git commit <文件>`，别裸 `git commit`）：共用工作树里别人随时可能有已暂存的改动，
  裸提交会把它们一起带走 —— 那就是 #296。
...
⚠️ **这一族的名字是「组合树是对的，像素是错的」**（cc-Yxi_Entertainment 2026-09-08 归纳，他手上还有两个先例）：
- **#288**：十连结算页立绘 + 九张小卡塞在不滚动的 Column 里，大字体下第 10 条**在屏幕外**。组合进了、状态全对、编译过、审查也过 —— 截图一看就露。
- **#329**：擦拭遮罩的霜做成了不透明 —— 表现是「擦之前一片空白灰」，而代码逻辑完全正确（「盖住未擦区域」读起来天经地义）。
- **本条**：预览进了组合、状态也对，就是没有位置。
共同点：**这类问题的证据只存在于像素里**，而我们默认的验证手段（读代码、单测、子代理审查、编译）**一个都不看像素**。
所以规矩不该只管桌面版：**任何改了界面的东西，合并前把它真画出来看一眼**
（手机端上模拟器截图，桌面版照 #327 的 Xvfb）。

⚠️ **「画出来看一眼」只挡得住第一层，还有第二层：看得见 ≠ 用户看得见。**
`#329` 的后半段是这个：霜改成半透明之后，作者看截图觉得「能看见」，取了 alpha 0.72；
子代理去量才发现对比度只有 **1.55:1**。**作者知道那儿有颗星，所以他看得见；用户不知道。**
所以两条规矩是**接力不是重复**，别并成一条：
① 先**画出来**——挡住「东西根本不在」（位置、存在，#288 / #328 这一族，看一眼就露）；
② 再**量**——挡住「东西在、你看得见、用户看不见」（可见性 / 可读性：拿前景最暗处和**实际渲染的**背景算对比度跟阈值比）。
第一条靠看能解决，第二条**靠看解决不了**——因为看的人是知道答案的那个人。

⚠️ 顺带两条方法（同一个人的经验）：**要量「多久」就录屏，要看「是什么」才截图**（#306）；
**逐帧量像素比肉眼看可靠** —— 他抽卡换卡那次白闪只有两帧，肉眼绝对看不见，是量出来的。
`import -window root` 连着截也能逐帧，桌面版同样适用。

## #329 遮罩做成不透明 = 「擦之前一片空白灰」；改半透明之后浓度还得**量对比度**才算数（cc-Yxi_Entertainment，2026-09-07）

- **症状**：抽卡的「擦星星」，老板要的是「擦拭的前面是灰黑星星」。我第一版把没擦到的格子
  用底色**不透明**盖住 —— 于是擦之前整屏是一块空白灰，那颗星星根本看不见。
  **代码逻辑完全正确**：底图画了、格子算对了、擦开也确实露出图。是"对的代码画出错的画面"。
- **第二次没修干净**：改成半透明之后我取了 `alpha = 0.72`，自己看截图觉得"能看见"。
  子代理去量了才发现：星星最暗处 `#4F4242`、底 `#E2E0DF`，原图对比度 7.29:1，
  蒙上 0.72 之后只剩 **1.55:1** —— 室内勉强看出轮廓，日光下等于没有。
  参考值：0.60 → 1.92:1 · **0.50 → 2.35:1**。最后取 0.50。
- **根因**：两层都是「凭感觉判断可见性」。
  · 第一层：不透明遮罩在代码里读起来是"盖住未擦区域"，很合理 —— 只有画出来才知道它盖掉了主体。
  · 第二层：截图上"我能看见"不等于用户能看见。看的人知道那儿有颗星、屏幕亮度也不一样。
- **修法 / 怎么避开**：
  · **可见性是可以算的,别投票**：拿前景最暗处和背景实际渲染色算对比度，跟阈值比。
    正文字号的门槛是 4.5:1，装饰性图形宽松些但 1.55:1 在任何标准下都不够。
  · 一般规律：**「逻辑全对、看着像坏了」这一类，读代码和单测永远发现不了** ——
    证据只存在于像素里（同 #328）。界面改动合并前必须真画出来看一眼，
    而"看一眼"若涉及可见/可读，就要**量**，不要凭印象。
