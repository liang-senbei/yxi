...
- **cc-Yxi_pilot · 网址预览卡跟在对应段落下面**（老板 2026-09-07 报：「网页渲染最好紧紧跟在对应的链接下面而不是统一放最下面」）：
  `agent/MdChunks.kt`（新，按空行切块但**绝不切断围栏代码块和列表** —— 松散有序列表被切开会让编号从 1 重来）· `ui/ChatScreen.kt` 的 AssistantText 改逐块渲染，每块摆上**本块首次出现**的网址卡（全消息仍封顶 3 张、跨块去重）· `androidTest/MdChunksTest.kt`（新，13 条）。
  审查 fix-first 三条已修：引用式链接定义被切散会渲染成纯文本（定义补进每一块）· 块间距 0dp（外层 Column `spacedBy(8.dp)`）· CRLF 让空行不算空行导致**功能静默失效**。
  （**opus 审查 ✓ · 编译 ✓ · i18n 归零 · 仪器测试 34 条 ✓ · Mac E2E ✓**：真会话里卡片跟在「2. 官方规约对音游说得很直接」那段下面、末尾不再堆卡、段间距均匀）
- **cc-Yxi_Entertainment · 抽卡出货重做**（f15ef34）：老板连退三次后定版。通用蓄力段接回来（28.5 秒素材原来只用了 2.4 秒）· 蓄力的**光晕跟着这一抽的品质走**（色相旋转，只染光晕和发冠、不染背景人物）· 擦拭期间只有灰黑星星、没有任何品质色 · 越稀有揭晓越长（蓝 1.09s → 红 1.96s）· 片尾在视频里烤 0.45 秒淡入结算卡底色，换卡看不出接缝 · 声音自己合成（原片音轨是数字静音，`design/music/make_gacha_sfx.py`，numpy 无采样包）。素材 585KB → 3.3MB，删掉不再用的五张 webp。
  审查抓出七条全改，三条是真会炸的：**收尾计时用墙钟**跟播放器无关 → 短片每次被砍掉尾巴、片尾淡入正好被切掉（#294）· **两个 Dialog 直接 if/else 换**中间一两帧谁都没盖住 → 录屏量到两帧 `#FCF9FC` 白闪（#295）· 霜的浓度实测对比度只有 1.55:1，星星等于看不见。
  （**opus 审查 ✓ · 编译 ✓ · Mac 模拟器 22 秒录屏逐帧核对 ✓**：擦拭屏星星可见且无格纹 / 擦完立刻是品质色 / 短片完整播到淡出 / 换卡全程无白闪且四周与画面同步 / 点一下能跳过）
  ⬜ **声音真机没听过** —— 模拟器是 `-no-audio` 起的，这台机器上永远验不了。
