...
- **cc-logto_yxi · 上传第三轮修复(1fe895d)**:老板 2026-09-06「一次传很多图片 / 传视频还是失败」。真根因是**永远转圈不是报错** —— jsch 在无超时的管道流上等 SFTP 应答,而它自己的 keepalive 此时也发不出去(线程栈实证,TROUBLESHOOTING #277)。修:60 秒没进度就关通道/断连接、下次断点续传;重试抽成 `agent/Uploader.kt`,分享面板也走它。**已过 opus 审查 + Mac mini 实测**(不靠限速的确定性故障注入:等重连 ✓、传到一半掐断后续传且大小一字不差 ✓)。对话页(f748d33)、分享面板、压力测试现在走**同一份** `Uploader`。

- **cc-Yxi · 侧边栏标出走内网的主机**（老板 2026-09-06）：抽屉里主机行加「内网」药丸，只按**这台手机怎么连它**判（`Host.viaTailscale` = 勾了内网且填了 100.x）。⚠️ 不是「这台主机有没有内网设备」——hk13 底下挂着一堆内网机器，但手机是走公网连它的，**不标**；tailscale测试 是手机走内网连的，**标**。**药丸改成描边**（审查打回，原来用 `tertiaryContainer` 填充）：项目的 `Theme.kt` **根本没定义 `secondaryContainer`**，而浅色下 `tertiaryContainer` 恰好 = `SurfaceContainerHigh` = 抽屉底色，对比度 **1.05:1，等于没有**；我第一次"实测通过"是因为只截了**选中**那一行（紫底上勉强看得出），没选中的行上基本隐形。这一行有两种底（抽屉底 / 选中行），**任何填充色都会在其中一种上糊掉，所以用描边**。
  **编译过 · i18n 归零（「内网」En 里已有）· Mac 模拟器实测过三台主机对照**（Hyw / hk13 无标记、tailscale测试 有）；**描边版的复验还没做**（要专门看没选中的那一行）。
- **cc-Yxi · MCP 认证：回填没反应 + 让 Yxi 能接回调 + 按钮文案**（老板 2026-09-06 截图报的）：① 「交上去」点完输入框和按钮一起消失、上面还写着「等浏览器那边授权…」，**完全看不出交没交上** —— 加中性反馈 `Flow.note`（跟红色的 `hint` 分开），并且先 peek 一下服务器那头的会话还在不在，没了就直说「流程已结束，关掉重来」（会话没了时 `send-keys` 是静默空操作，用户会一直等到 10 分钟超时）。② 有些浏览器（荣耀自带 / 应用内 WebView）不自己跳 localhost，而是弹「使用以下方式打开」让你挑 App，选单里没有 Yxi → 只能手抄地址回填。加 manifest intent-filter（`http` + `localhost` + 路径钉死 `/callback`，不写端口 = 任意端口），MainActivity 收到就塞进 `Connect.pendingRedirect`，正开着的流程在「等授权」那步取走 = 替用户按了「交上去」；`Flow.init` 里清掉陈旧回调，免得没开框时收到的地址被下一个流程吃掉。③ 文案：「先关掉」→「关掉」、「去浏览器授权」→「认证」、「先关掉（后台继续装）」→「关掉（后台继续装）」。**Opus 审查过，6 条全修**（这条改动最值得记的是前两条）：
  - **① 自动提交是错的（高）**：那条 intent-filter 是**导出**的，手机上任何 app 都能拿构造的 `http://localhost:*/callback?code=…` 唤起我们，而原来的写法**零交互**就把它送进服务器会话，且不校验端口/是不是我们等的那次。改成**只填进输入框，由人按「交上去」** —— 省掉手抄，又留一道人眼。（审查专门验了转义：`a'"'"'; touch /tmp/PWNED; echo '"'"'`、`$(…)`、反引号、换行四种真跑过，全部原样落地，**没有命令注入**，`shq` 的单引号包裹是对的。）
  - **② 「空」当成「结束了」（高，跟 #202 同款）**：我加的「先探一下会话还在不在」用「抓屏返回空 = 流程结束」判断，但 `SshSession.exec` **连接断了也返回空串**（`ssh/SshSession.kt:357`），两者字节级相同。后果：锁屏/切网后点「交上去」→ 被告知「流程已结束，关掉重来」→ 一关就把好好的会话真杀了。**这个防呆自己制造了它要解释的故障。** 改成让命令自报家门（`Connect.aliveCommand` → `__ALIVE__` / `__GONE__`），**空 = 不知道，照发，让 waitDone 判**。
  - ③ 回调在「准备中」那几秒到达会被留着、几秒后步骤翻到 Authorize 时自己冒出来（那多半是上一轮的废码）——改成收到就立刻收走。
  - ④ `setIntent` 从没调过，切深浅色/转屏重建 Activity 会把同一个回调再放一遍 —— 消费完 `i.data = null`。
  - ⑤ 回调回来但连接框已关会被**静默吞掉** —— 改成明确 toast「这次白跑了，重新点连接」（那串码确实是废的：重开流程会 kill 旧会话）。
  - ⑥ 见下面「侧边栏内网标记」那条。
  **编译过 · i18n 归零 · Mac 模拟器验过 intent-filter 注册上了**（`cmd package query-activities` 里出现 app.yxi.MainActivity；打这个 URL 进 App 不崩）。**修完的复验还没做**；「认证 / 关掉」两个按钮在真 OAuth 流程里的样子也没截图（要一个活的 MCP 服务才走得到）。
- **cc-Yxi · 「我的」页改造 + 侧边栏主机段可收起 + 两个模型 bug**（老板 2026-09-06）：
  ① **「我的」分块**（账号 / 资产 / 功能）：账号组 = 资料卡 + 会员中心 + **新的「账号中心」**（邮箱 / 登录方式 / UID，卡片上直接显示邮箱）；个性化从会员和钱包中间挪进「功能」组。
  ② **点头像进「我的资料」**（新页 `ui/ProfileScreen.kt`，只读；改昵称签名头像还是走 MeDialog —— 昵称存本机 prefs、会员邮箱来自服务端，混一页里编辑容易写花）。拿不到的字段**整行不显示**（空签名、Free 无到期都验过）。
  ③ **账号中心**（新页 `ui/AccountScreen.kt`）：登录方式来自 logto 2026-09-06 上线的 `signInWith`（契约 `logto_yxi/design/account.md`，取值 email/google/github，**以后会加**）。⚠️ **不写穷举 when**：认不出的值整行不显示，不写「未知方式」。
  ④ **侧边栏主机段可收起**（老板：「服务器多了会不会不方便」）：**超过 5 台默认收起**只留当前那台、标题写「N 台 ▸」；5 台以内默认展开。顺带**删掉功能区那行重复的「主机」**（老板：「怎么有两个主机」），换成列表末尾一行「管理主机」。
  ⑤ **顶栏模型名跟不上 `/model`**：`lastCtx` 是 `parseInto` 的局部变量、**每批解析重置成 null**，而 `/model` 回执常单独成一批 → 「切了但还没回话」那一支被丢掉。改成跨批传下来。⚠️ 原有用例把回话和回执放在**同一个 add()** 里，所以一直是绿的；新增 `TranscriptTest.回执单独一批也认` 按分批写。顺带摘掉模型名里的反引号和 `(default)` 后缀。
  ⑥ **换模型不显示「默认」**：`av.default` 来自 settings.json 的 `model`，**没写过就是空的** → 整张表一个标记都没有；且 `availCache` 是进程级的、切完不重读。改成空时「默认」落在 `default` 行、打开选单 `force = true` 重读，并加一句说明 default 是什么。
  **Mac 模拟器实测**：我的页分块 / 点头像进资料页 / 账号中心显示邮箱+「邮箱注册」/ 7 台主机默认收起「7 台 ▸」/ 邮箱过长折行不顶边，全过。
  **⚠️ Opus 审查（补跑，11 条：2 高 3 中 6 低）—— 其中两条是这次改动自己制造的回归，值得记住**：
  - **高｜「账号中心」对着登着的用户写「还没登录」**：LoginGate 把没登录的整个 App 都挡住了，走到那页 `me` 仍为 null **只可能是 `/api/me` 没回来**（首装 / 离线 / 5xx）。这是 **#240 原样重犯**，而修法就写在我改的那个文件隔壁几行（SettingsScreen 的「登录了但资料还没回来 → 已登录，别写未登录」）。改成「资料还没拉回来，稍等一下。」+ 触发一次 refresh。
  - **高｜跨批修复把一个隐患激活了**：顶栏判「上下文吃紧」靠模型名里有没有 `[1m]` 区分 1M / 20 万；而 `/model` 回执给的是**显示名**（`Opus 5 (1M context)`，不含 `[1m]`）。**修好之前回执被丢掉所以不发作**，接通之后 → 在 1M 模型上切一次模型，上下文条立刻误报「吃紧」。判据补成 `contains("[1m]") || contains("1M")`。
  - **中｜跨批只修了三分之一**：`mode` / `ponytail` 当时是塞进 `Ctx` 捎带的，而 `Ctx` 要见到带用量的 assistant 行才存在 —— 「开着计划模式进对话页」那行 mode 在更早的批里，照样死在批边界。改用独立的 `Transcript.Carry` 带，并补 `TranscriptTest.模式和强度也跨批`。
  - 中｜账号中心那行邮箱**没给 weight**，Compose 先量无 weight 的孩子 → 邮箱吃掉整行、标题折行甚至连 `›` 都挤没。
  - 中｜开选单时 `force = true` 重读 settings.json，把「零往返立刻弹」变成等 SSH（老板报过「选单延迟很高」），抖一下还会用通用四项覆盖真实 availableModels。正解是**切完就作废缓存**（`Model.switchFast` 里 `availCache` 清空），不是每次重读。
  - 低｜模型名剥壳顺序脆（反引号 / ANSI / `(default)` 嵌套顺序不固定）→ 改成反复剥到不动；「默认」标记在「表里没有 `default` 行」时仍然一个不标 → 退回标首行；`onEdit` 是死参数；`GroupLabel` 与已有 `SectionLabel` 重复；「我正在用的装扮」尾巴写「去装扮」（标题承诺状态、尾巴给动词，等于没信息）→ 改显示真在戴的那款。
  **修完复验**：TranscriptTest 6 条全过（含两个失败输入）· 账号中心标题不再被挤没 · 装扮行显示「不戴」。**编译过 · i18n 归零。**⑤⑥ 的真机确认（真 Claude Code 上切模型不发消息看顶栏）仍没做 —— 桩太薄走不到。
...
- **cc-Yxi_Entertainment · 个性化 → 装扮 → 聊天气泡**（老板 2026-09-06 发了 QQ 聊天气泡商店截图）：「我的」加「个性化」入口 → 分类（聊天气泡 / 头像框 / 终端配色 / 快捷语包）；气泡从「只有底色」升级成一套样式（边 · 尾巴 · 描边 · 玻璃 · 角标），新增六款：心理活动框 / 云曦想想（复刻老板发的那两款 QQ 气泡）/ 素描边 / 云朵 / 玻璃 / 像素。画法只有 `ui/Bubbles.kt` 的 `BubbleBox` 一处，对话页、商店预览、装扮小样共用。**已提交 d4e3abe**（按 hunk 提，没带走别人未提交的改动）。六个 id 已进祈愿奖池（cc-logto_yxi，wish.json 2026-09-06a：紫档 心理活动框 / 云曦想想 / 云朵，蓝档 素描边 / 玻璃 / 像素）—— **客户端和奖池必须同版进**，先发客户端会出现「抽不到的气泡」，先发奖池会出现「抽到了画不出来的气泡」。**Opus 审查 ship · 编译过 · i18n 归零 · Mac E2E 过**（四个分类都进得去 · 商店六款齐 · 预览 / 使用 / 使用中 · 对话页真气泡两款都对，用的是 cc-Yxi 搭的 cc-demo 会话）。齐了。
- **cc-Yxi_Entertainment · 活动 ·「云曦节拍」音游**（老板 2026-09-06：「活动里做一个 Phigros / Orzmic 那种的，顺便调研歌能不能用」）：活动中心新增入口 → 选曲（三首 × 两档难度）→ 一条判定线 + 四轨下落（Tap / Hold）→ 结算（分数 · 准度 · 评级 S/A/B/C · 最高连击）。**曲子是我们自己合成的**（`Yxi_Entertainment/design/music/make_songs.py`，numpy 现算波形，没有采样包 / SoundFont / 第三方音频；六张谱从同一份乐谱导出，天生对齐），词曲和录音两层权利都在我们手上 —— 别人的歌一首都不能用，理由和四条可行路线写在 `Yxi_Entertainment/design/rhythm-prd.md` 第 1 节。**分数不是客户端算的**：客户端只报判定计数，服务端按同一套公式算分发奖（契约 `logto_yxi/design/rhythm.md`，cc-logto_yxi 已上线）。⚠️ 两个字段别混：**评级读 `bestRating`（历史最佳）、分数读 `best.score`（最高分那局）**，背离是正常的；混用会把已经拿到的 S 显示成 A。⚠️ 谱面以后要改（跑 make_songs.py）**必须先告诉 cc-logto_yxi**：units 变了而 rhythm.json 没跟，所有提交会被判 bad_counts。提交 `d234aa7`（主体，cc-Yxi 代提）+ `34c0a90`（审查两修）+ `cda4d60`（全 S 限定气泡 `bubble_beat` 的画法；顺手修一句假话：商店对未拥有的款一律写「祈愿可得」，但活动限定款不在奖池里，现在写清出处）。**Opus 审查 ship · 编译过 · i18n 归零 · Mac E2E 过**（入口 → 选曲 → 打完一局 → 结算 → 上传服务端 → 成绩回填；服务端的判定数校验当场抓到一个长按尾判漏掉的真 bug，已修；切后台复验停在选曲页）。齐了。
- **cc-Yxi_Entertainment · 云曦「任务看板」**（老板 2026-09-06 发了 omggrow 的看板截图：「提醒我们还有什么东西没做，方便随时记」）：备忘录升级成看板 —— 四态（待办 / 在做 / 受阻 / 已完成）筛选条带计数、快记一行时顺手挑优先级 / 截止（今天 · 明天 · 选日期）/ 提醒 / 重复，每条能改状态改优先级、逾期标红、云曦的话会念「还有 N 件没做」。文件：`ui/YunxiScreen.kt`、`ui/En.kt`（+22）；数据层 `agent/Memos.kt` 是 cc-Yxi_pilot 的（他自己记，须同版进）。⚠️ 看板的「在做」**不能**复用 `t("进行中")` —— 那句在工具卡片里是「Running」（跑着的工具），看板上英文会变成 Running；审查揪出来了，已改成独立的「在做」= Doing。**Opus 审查 ship · 编译过 · i18n 归零 · Mac E2E 过**（空看板 → 记一条 → 展开改状态 → 完成 → 开场白念未完成数）。齐了。
- **cc-Yxi_Entertainment · 云曦天气「用当前位置」**（老板 2026-09-06「不能用手机定位吗」）：天气卡选城市处加「用当前位置」（先申请粗略定位权限 → `WeatherApi.locate` → 存城市不存坐标 → 刷天气；拿不到按原因如实提示并保留手动选城市；注明「只用一次粗略位置，不保存」）。文件：`ui/YunxiScreen.kt`、`ui/En.kt`（+6）。服务层 locate/权限/搜索修复是 cc-Yxi_pilot 的（他自己记，须同版进）。**Opus 审查 ship · 编译过 · i18n 归零 · Mac E2E 过**（权限弹窗 → 允许 → 模拟器拿不到位置时提示正确；成功路径待真机）。齐了。
