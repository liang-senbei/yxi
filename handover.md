...
**1.1.21 之后攒的**：
- **cc-Yxi_Entertainment · 站内信 30 天清理（客户端）+ 修「兑换码从来没显示过」**（264ea06）：老板派的保留期规则，服务端已上线，这是客户端那半 —— 列表行**临期才**显示「还剩 N 天」（带码的信提前到 14 天，那种删了只剩截图），展开处按**四种组合**说清「会不会亏」。天数来自 per-mail `expiresAt`，**没写死 30**。**顺带修掉一个功能上线起就在的哑 bug**：兑换码的码在附件 `code` 字段里，客户端从来没解析过，界面显示和长按复制的都是 `name`（「兑换码」三个字 / 商品名）—— 是新写的「先抄下来」这句文案把它照出来的（见 TROUBLESHOOTING #303）。**过了**：opus 审查九条（前三条必修全改）· Mac 模拟器真接口 E2E（在服务器发了三封测试信覆盖四种组合，码正确显示可复制）· i18n 归零。**没过**：「随时会清理」和 ≤3 天转红没见过（要真过期的信）。
- **cc-Yxi · 狂热演示只在无线时刻半空爆**（老板 09-07 二次：「应该只有校准线消失才可以随便校准」；a1da023）· **爆点 / 碎裂在线立起来时不压扁**（22f522a）· **清单刷新改 object 级 scope**（页面一走谱下到一半就断，d822dd9）· **游戏 HUD 得分左边放头像**、**「我的」加「娱乐中心」入口，云曦节拍从活动中心挪过去**（功能格改成两行三格；老板 09-07）。同一提交里的谱面改动（跟节奏的狂热谱、分裂线）**已经走网络发出去了**，不用等发版。

- **cc-Bug_solverYxi · 抽卡黑屏真根因 + 看得见的「跳过」+ 通知换成新 logo + 单字「找」改 🔍**（老板 2026-09-07 三条）：黑屏是 `prepareAsync()` 之前 `use { }` 把 fd 关了、部分真机**静默失败**（根因 cc-Yxi_Entertainment 定位，详见 TROUBLESHOOTING #305）；顺带补了「起播没来」的兜底和 `isOpaque=false`。跳过**逻辑早就有**（点哪儿都能跳），缺的是看得见的入口，加了个「跳过」；不点它仍走完整流程。通知大图标 `ic_notif_large` 还是旧的 Yunxi 手写体 → 换成新 logo（花瓣，底色跟启动器图标同为 #F4F6FA）。`YunxiScreen` 里那个单字「找」按钮 → 🔍。（**编译 ✓**；⚠️ **黑屏本身模拟器复现不了**——它容忍关掉的 fd，只能老板真机确认）
