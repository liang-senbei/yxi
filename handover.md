...
| # | 决定 | 理由 / 出处 |
|---|---|---|
| D27 | **实验室要规范：App 只按一套卡片模板显示；内容格式、尺寸、审核全走 `yxi-lab` 这一个接口（`spec` / `template` / `check` / `add --aspect`）；以后 AI 生成新内容只推、不改 App** | 用户 2026-09-02：「实验室需要规范，模板和 UI/UX 做好，给出 API 接口方便后面的 AI 生成上传展示，不要每次改一遍实验室」。契约在 `server/LAB.md`（= `yxi-lab spec`）。 |
