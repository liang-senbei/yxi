...
**1.1.17 之后攒的**：
- **cc-Yxi_Entertainment · 抽卡出货换成「擦星星 + 短片」**（d6e6687 / 3921e6d）：按老板 2026-09-06 的九宫格设定，出货先擦开一张遮罩、擦完接一段 0.6 秒短片（红/金/紫/蓝各一段，从老板自己录的无水印版本里裁的）。十连只擦一次，用这一批里最高的那档。**过了**：opus 子代理审查、Mac mini 模拟器 E2E（单抽 / 十连 / 关动画跳过）。**没过**：真机没看过。
- **cc-Yxi_Entertainment · 祈愿逐项查一遍**（7aff8f1）：曦光不够改成明说差多少去哪拿（原来只把按钮变灰）；十连结算页改成可滚（大字体下第 10 条在屏幕外）；保底那一发标出来（服务端新加 `results[i].byPity`，cc-logto 那边 hk13 已上线）；卡牌库「已收集 27 / 9」改成只数角色。**过了**：opus 子代理审查（七条改了六条，第七条也照办）、模拟器 E2E（单抽 / 十连 / 记录 1 条与 10 条 / 概率公示 30 档合计 100.00% / 卡牌库 9 / 9 / 关动画 / fontScale 1.0 与 1.5）、`dev/i18n-check.sh`。**没过**：真机没看过。
