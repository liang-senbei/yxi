...
**1.1.24 之后攒的**：
- **cc-Yxi · WildBg 纯色硬切换色频率钉死 2.5Hz、关闪屏开关时 0.5Hz**（Entertainment 09-07 指出：客户端自闪 4Hz 超光敏阈值且不受开关管；5c303c3，已编译）。

- **cc-Yxi_pilot · 从机（主机 → 跳一层看另一台机器）**（老板 09-07：「服务器当主机、Mac 当从机，点进从机去看它活着的会话」「要放行 SSH 权限的话你要提醒他并指导」）：
  主机卡片展开 →「内网设备」→ 在线的那几台可以点进去（`ui/SlaveScreen.kt`），列出它上面的 tmux 会话，点一个**在终端里接上去**（`Slave.attachCommand` → `Workspace(bootCommand)`）。
  连不上时**不只说「连不上」**：`agent/Slave.kt` 一趟往返分五种因，每种给「怎么了 / 为什么 / 怎么办」+ 可复制的命令 ——
  tailnet 的 ACL 挡了 22（给可粘的 ACL 规则）· 机器没醒 / 没开远程登录 · 主机的公钥没装进从机（`ssh-copy-id`，文案强调是**主机的**公钥不是手机的）· 指纹变了（`ssh-keygen -R`）· 看不懂就给 `ssh -v`。
  顺手修的真 bug：**Mac 当主机时「内网设备」永远报「没装 Tailscale」**（`agent/TailscaleStatus.kt`，App Store 版的 CLI 只在 app 包内，见 #311）。
  （**审查 ✓（opus，10 条，全改了）· E2E ✓**：Mac mini 当主机，点 hk13-new → 正确报出 ACL 那一支并给规则；点一台够得到的 → 列出 `cc-macdemo` → 点进去终端真的接上了那个已存在的会话。全套 306 条跑过，红 0（UploadStress 那 7 条要显式传 stressKey 才跑，平时是跳过 —— 见 #316）。）
  · 审查抓到的重头：**命令注入** —— `Shell.q(会话名)` 插进了一段已被单引号包住的字符串，它自己的开引号把外层闭掉，
    从机上一个叫 `x;id;#` 的会话点一下就在**主机**上执行 `id`（已改成「内层按从机语义拼好、整体再引用一次」+ `parseSessions` 补 `safeName`）；
    探测连的是**当前选中的主机**而不是那台设备所属的主机（长按展开不等于选中）；探测接的是抓好的 session 而不是 `rememberAliveSsh`，
    手机自己掉线会被报成从机的毛病；`contains("host key")` 把 `no matching host key type` 误判成「指纹变了」。
  · E2E 又抓到一个单测抓不到的：**zsh 的 `=` 展开**吃掉了 tmux 的精确匹配（见 #314）。
  · 未做（v2）：主机健康卡在 macOS 上读不出（走的是 `/proc`，Mac 要换 `sysctl`，界面现在显示「读不出这台机器的状态」）；「文件」页还不能穿到从机上。

- **cc-Yxi_pilot · 长按「对话」的历史改成落文件、留三天**（老板 09-07 傍晚，经 cc-Yxi 转达）：`ui/SentLog.kt`，
  每个「主机 + 会话」一个 jsonl，**发成功**才记（发失败的话已经还回输入框了，记了就是骗人），一行一条 JSON（正文带换行/引号不串行），
  面板里新的在上、显示时间和附件数；三天前的在**发送和打开面板**两个时机顺手清掉，不另起后台任务。
  ⚠️ 为什么不接着从转录里滤：转录够不到远处（会话一长前面被截、`/clear` 之后一条不剩），而翻历史找的恰恰是远处那条。
  （**审查 ✓（opus）· E2E ✓**：模拟器上发两条 → `files/sent/<base64>.jsonl` 两行 → 长按「对话」弹出「发过的话 2 条」，新的在上、带时间戳。13 条单测覆盖分会话/分主机、怪名字、换行引号、附件计数、三天过期、坏行、空话、追加语义、老格式迁移、扫全盘。）
  · 审查抓到的重头：写法是「读全表 → 过滤 → **整份回写**」——`writeText` 先清空再写，
    **每发一条都开一次「写到一半被杀就全丢」的窗口**（国产 ROM 后台杀得凶）；读一次失败更是把「只有新这条」写回去，
    瞬时错误变永久全丢。改成**只追加**（一次 O_APPEND），压缩挪到读的时候做、走 tmp+rename。
    ⚠️ 连带一个升级坑：上一版写的文件**末尾没换行**，直接追加会粘行 —— 追加前看一眼末字节。
  · 另外三条：**通知里回的那句、分享进来的、会话列表「回它一句」都没记账**（老板要的是「每一条」），三处都补了；
    「三天后清理」原来只对「你正在打开的那个会话」成立，不再打开的会话和删掉主机后的孤儿文件会**永远留着** ——
    开 App 扫一遍 `filesDir/sent/`；点一条历史原来会**把正在打的草稿冲掉**，改成接在后面。
  · 不加密（审查的结论，有据）：整份转录本来就明文躺在 `cacheDir/transcript/`（`TranscriptCache`，还没有过期），
    `Vault` 又是 `setUserAuthenticationRequired(false)`，只挡关机取盘 —— 而那层 FBE 本来就有。真正的缺口是保留期，已经补上。
