...
- **cc-Yxi · 老板 09-07 下午一批**：12345 狂热谱（`frenzy_chart.py`，units 851，logto 已同步服务端）· 狂热演示半空提前爆 · 曲子能量包络驱动底光（更亮、色相拉开、多两团、四边压暗）· 命中闪加强 · 选曲页音块图例 · 判定线整条留在屏幕里（大角度外侧轨点不到）· 难度名 / 谱数按每首的 diffs 算。**过了**：opus 审查（抓到跳档必崩的数组越界，已修）；模拟器狂热演示（截图 + logcat 崩溃数 0）。**多判定线**（全局轨号方案，狂热谱 4 个副线窗口）。**关卡联网**（清单已公开，17 张谱服务端已同步；规格 §6.4）。
