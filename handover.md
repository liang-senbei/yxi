...
第四十八轮：插件历史按主机隔离、最近在前，支持搜索/查询/恢复入口与可见滚动条；拒绝回执不会因查询失败变成未知阻塞主机。桌面测试及打包通过，软件渲染原生滚动/关闭验证通过（fixture.YRl5RH）。默认Linux渲染再次退出卡住，记录为未解决；仅测试脚本改默认SOFTWARE_COMPAT。历史恢复点击、缺失记录对账及Windows专项仍待完成。
...
第三十九轮：目录→范围→核对→取消→确认安装的原生SSH流程通过，核对CLI识别及客户端installed持久状态；目录空状态改卡片并提供主机插件入口。固化脚本加入超时边界。一次Linux测试JVM在halt0退出卡住，诊断后终止该worker，后续完整复跑正常，根因待查。136项中128通过、8环境跳过，Linux打包通过。Windows/项目范围/远程来源及权限专项继续推进。
