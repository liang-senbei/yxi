...
**1.1.20 之后攒的**：
- **cc-Yxi_Entertainment · 抽卡底部的播放器进度条 + 卡牌库命座**（7f69e23）：① 老板真机看到抽卡界面底部露出「00:00 / 00:15」—— 素材是他**录屏**的，播放器控制栏占了原片底部 82 行，且**只在每遍开头 3 秒可见**（中段干净，所以逐帧翻看不出来）。四条 mp4 + 擦拭图重出、crop 到 608×918，**只裁不补**（各档底色不同，纯色补不上）。② 老板问「重复的红色角色怎么看命座」—— 原来只有深渊配队页有，卡牌库补上右下角标，服务端给的是**封顶值**。**过了**：opus 审查五条全改（含「starCenter 的 0.52 是按旧分母标的、改比例后光晕打到脸上」和「LaunchedEffect(Unit) 导致抽完看不到新命座」两条真会炸的，见 TROUBLESHOOTING #298 #299）· Mac 模拟器真接口 E2E（底部无进度条 / 命座 5·1·2 显示正确）· i18n 归零。**没过**：「命座 N · 满」变体没在屏幕上见到（测试号最高 5 命）。
- **cc-Yxi · 老板 09-07 下午一批**：12345 狂热谱（`frenzy_chart.py`，units 851，logto 已同步服务端）· 狂热演示半空提前爆 · 曲子能量包络驱动底光（更亮、色相拉开、多两团、四边压暗）· 命中闪加强 · 选曲页音块图例 · 判定线整条留在屏幕里（大角度外侧轨点不到）· 难度名 / 谱数按每首的 diffs 算。**过了**：opus 审查（抓到跳档必崩的数组越界，已修）；模拟器狂热演示（截图 + logcat 崩溃数 0）。**多判定线**（全局轨号方案，狂热谱 4 个副线窗口）。**关卡联网**（清单已公开，17 张谱服务端已同步；规格 §6.4）。⚠️ durationMs 口径改成曲子真实时长后，服务端 too_fast 门槛整体抬高（logto 09-07）：客户端 elapsed ≈ 3 + 曲长 − 0.8 秒，仍高出门槛 10% 以上，模拟器整局已验；**真机最长的 count_frenzy 一局待验**。

- **cc-Bug_solverYxi · 「配置」和「我的」也能下拉刷新了**（老板 2026-09-07：「会话和主机都支持了，配置和我的也要」）：照 HostsScreen / SessionsScreen 那套 `PullToRefreshBox` 接的。配置 = 重读 `ConfigRemote.load`（**不清空旧的**，免得整页闪成转圈；空状态也能拉——那正是最想重试的时候）；我的 = 重拉 `/api/me` + 装扮归属，没登录就只转一下不空跑网络。（**编译 ✓ · 模拟器实机验过**：两页下拉都出转圈、内容不闪空、转完自己收）⚠️ 一开始只给「Agent 配置」那个面板接了，等于白接 —— **进「配置」默认看到的是「连接」面板**；现在三个面板（线路 / 连接 / Agent 配置）共用一个下拉，各自按 `refreshKey` 重读。
