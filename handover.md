...
**1.1.23 之后攒的**：
- **cc-Yxi · 「癫狂」难度的舞台特效**（老板 09-07 傍晚 I Wanna 视频）：谱面 `stage` 关键帧（zoom / sx sy / spin / shake / flash / bg / notes / hue），镜头套在场地外面、手指反变换、减弱动效不闪；`ui/rhythm/WildBg.kt` 四种背景；17 首的 wild 谱已走网络发布（老客户端看到「癫狂」按钮但没特效，音符同 hard）。
...
**待办（不急）**：
- **长按「对话」的历史改成落文件、留 3 天**（老板 09-07 傍晚：每个会话一个文件 / 文件夹，发送时追加，长按直接读文件，每条保留三天后清理）—— 已派给 cc-Yxi_pilot（那是他的功能），做完进待发版。
