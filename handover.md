...
**1.1.25 之后攒的**：
- **cc-Yxi · 连击边框流光**（老板 09-07 晚：圈选屏幕那圈光；到第二档出现、档位越高越亮、命中亮一下；57e92e3）· **癫狂背景 20 种**（四个子代理各 5 种，`WildBg{A,B,C,D}.kt`，总入口 `WildBg.kt`；每张谱随机挑 3~4 种，17 张 wild 谱重出但**等发版后再 publish**——老客户端不认识新号）。
