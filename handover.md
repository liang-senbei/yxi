...
- **cc-Yxi_pilot · 主机加 Tailscale 内网连接**（老板 2026-09-05 要的）：加/改主机表单「公网 | Tailscale 内网」切换 + 长按主机「内网设备」列表（`tailscale status --json`）。已在 main（`3712acf` / `2c8ccf5`）。**Opus 审查过**（2026-09-06，2 medium + 2 low 全修，工作树里：`DevMode.kt` 诊断括号里「公网」误用 connectHost → hostname；`HostsScreen.kt` 拉设备列表被取消时 Session 漏 → try/finally；内网 IP 框补全角字符提醒；一句过 t()，En +1）· **Mac E2E 过**（设备列表列出本机在线 + Han 离线带最后在线时间；表单切内网出 IP 框、空 IP 保存禁用）· 编译过 · i18n 归零。
- **cc-Yxi_pilot · 云曦小管家 · 服务层**（与 cc-Yxi_Entertainment 的页面**同一版一起进**，单独进不了）：新包 `app/yxi/yunxi/`——`Memos.kt`（备忘 + 提醒一份数据，`filesDir/yunxi-memos.json`，Compose state）· `Reminders.kt`（`setAlarmClock` + `USE_EXACT_ALARM`；`ReminderReceiver` 到点发通知、BOOT / 升级后重排；重复按墙上时间推）· `Weather.kt`（Open-Meteo 免 key，手动选城市不要定位，30 分钟缓存）；`AndroidManifest.xml` +2 权限 +1 receiver。接口约定 `Yxi_pilot/design/yunxi-api.md`。**Opus 审查过（2 medium + 3 low 全修）· Mac 仪器测试 `YunxiTest` 6 条过 · 页面方 E2E 跑通（备忘 + 30 分钟提醒排上、搜城市 / 取天气 OK）· 编译过 · i18n 归零**。En 那 25 条已由 Entertainment 贴入。
- **cc-Yxi_pilot · 配置 → 连接：加「微信」+ 全部分类**（老板 2026-09-06「配置的链接加一个微信，顺便给链接里的都分分类」）：`agent/Connect.kt` 加 `Kind.LOCAL` + `Service.cat`，`ui/ConnectPanel.kt` 按类分组（Agent 登录置顶不分组 · 开发 · 协作 · 工具 · 本地 · 仅限订阅）。微信一行：状态 = 服务器上 `ssh laptop` 探 `C:\temp\decrypted_key.txt`（在 = 已连接 / 电脑不可达 = 连不上 / 没段 = ABSENT）；「连接」= 跑 `getkey.js` 提取密钥；「断开」= 删密钥。⚠️ 只在有 `laptop` 隧道别名的服务器上有意义（老板自用）。En +13。**Opus 审查过（1 low 已修）· Mac E2E 过 · 编译过 · i18n 归零**；仪器测试 `ConnectWechatTest`（5 条）待跑。读聊天记录的界面还没做，不在这一批。
- **cc-Yxi_pilot · 对话页点空白处切上下栏**（老板 2026-09-06「会话里点击屏幕就唤起上下的导航」）：`ui/ChatScreen.kt` 三处小改，点正文一下页眉 + 输入栏收起（正文不动）、再点回来；链接 / 按钮 / 卡片各自的点击不受影响，拖动不算，键盘开着不切；三段栏动画统一成 `animateBars` 起新掐旧。**Opus 审查过（1 low 已修）· Mac E2E 过（真会话）· 编译过**。
- **cc-Yxi · 话术去 AI 味 + 临时会话跟线路**（老板 2026-09-05）：① 遍历用户可见文案，去掉自辩式和解释实现的句子——「明天签到给：」→「明日签到奖励：」、「这块还没开通。界面先摆在这儿…不做点了没反应的假按钮。」→「功能开发中，敬请期待。」、「认证一次，这台机器上的 Claude Code 就能直接调用它…」→「认证一次，agent 即可调用。」等 8 处（中英一起改）；孤儿条目「更多活动还没上…」删掉（那个位置现在是深渊入口卡）。② 临时会话：模型/思考强度**线路里设了就跟线路**（读 settings.json 的 `model`/`effortLevel`，`Model.available` 顺带把 effortLevel 也读出来），没设才是 sonnet/medium；进已有的临时会话自动 `/clear`（刚开出来的那个跳过——本来就是空的，且 claude 还在启动）。**Opus 审查过**（1 critical + 2 high 全修：① `/clear` 的 LaunchedEffect 原来把 `shared.session` 放进 key，断线重连会换新 SshSession → effect 重跑 → 把用户正聊着的上下文清了（手机切网/息屏 30 秒即触发），改成只按「进了哪个会话」重跑、连接在里面 `snapshotFlow{}.filterNotNull().first()` 等；② 送 `/clear` 前补 `Model.borrowable()` 守卫 —— 输入框里有半截草稿会被接上一起发出去，跟 switchFast 同一个规矩；③ `Model.available` 的 availCache 是进程级的，换线路后不重读会拿旧模型起会话，改 `force = true`；另修 effort 非法值（如 "mid"）导致整排芯片不高亮、以及「跟当前线路」这个说法不准——那两个键 `/model`/`/effort` 也会写，改成「跟这台机器当前的默认」）。**Mac mini E2E 过**（判据是文件不是屏幕文字：测试机 = podman 容器 ubuntu24.04 + tmux + 假 claude 桩，桩把收到的每一行写进 /tmp/claude-in.log）：
  ① settings.json 写 `model:opus`/`effortLevel:high` → 弹窗两个芯片确实变 opus+high（不是默认 sonnet/medium），起出来的进程是 `claude --model opus --effort high`；
  ② 控制组：**新建**的会话桩日志里只有 LAUNCH，**没有** `/clear`；
  ③ 退出去再进：桩日志出现 `IN /clear`；
  ④ 负例：pane 里没有 Claude 提示框时 `borrowable=false`，**不发** `/clear`（instrumented 跑确认过）。
  E2E 又抓出两个代码 bug（审查没抓到）：`snapshotFlow{}.first{isAlive}` 在连接没活时会永远挂着（`shared` 是 data class，结构相等不发新值），改成轮询；`capture-pane` 返回空被当成「有草稿」而静默跳过，改成当失败重试。
  ⚠️ 建 rig 的坑记在 TROUBLESHOOTING。**编译过 · i18n 归零。**
...
- **cc-logto_yxi · 深渊服务端已上线**(老板拍板 PRD v1,2026-09-06):`GET /api/abyss`、`POST /api/abyss/floors/<n>`,契约 `logto_yxi/design/abyss.md`;opus 审查(1 条:取整 .5 向上,已修)+ 六套回归 + e2e 7k 全过。客户端页面归 cc-Yxi_Entertainment,他接上时自己记一行;服务端本身**不需要发版**。
- **cc-Yxi_Entertainment · 深渊（新模块）**（老板 2026-09-05 拍板：星穹铁道混沌回忆的大幅简化版，半月一期）：活动中心多一张「深渊」入口卡（本期没打过亮红点），深渊页 = 本期卡（塔名 / 距重置 / 紊流 / 星数进度 / 奖励说明）+ 12 层列表 + 配队 Dialog（每半 0–2 位、同层不重复、命中弱点 ✦、实时「预计」星数）+ 挑战 + 结果页（3 金 / 2 紫 / 1 蓝 借出货特效，落定一行字，跨档发奖写清到账的东西）。服务端 cc-logto_yxi 已上线（契约 `logto_yxi/design/abyss.md`，PRD `Yxi_Entertainment/design/abyss-prd.md`）。文件：`agent/Abyss.kt`（新）· `ui/AbyssScreen.kt`（新）· `ui/ActivityScreen.kt`（入口卡 + onAbyss）· `MainActivity.kt`（Page.Abyss + 路由，两行）· `En.kt`（+32）· `androidTest/AbyssTest.kt`（新，5 条）。**Opus 审查过（1 should-fix + 3 nit 全改）· 编译过 · i18n 归零 · Mac mini 仪器测试 8 条过 · 真接口 E2E 全过**（0 角色打第 1 层 2 星 → 发角色 → 配队 / 另一半锁定 / 预计与服务端结算一致 → 跨 3 星发曦光 ×2 + 站内信 → 入口红点消）。
  **追加（老板 2026-09-05 晚）**：① 独舞者 / 幽火 性格词补齐（孤高 · 灵动 / 寂静 · 不灭，`Crowns.kt` + 服务端 abyss.json 同步）；② 36 星限定装扮 = 星穹铁道「异相仲裁 · 王棋彩框头像」同款的**头像框**：新装扮槽位「头像框」（`ui/Skins.kt` FRAME · `ui/AvatarFrames.kt` 矢量绘制 · `ui/SkinPicker.kt` 加一栏 · `ui/Me.kt` MeAvatar 外圈叠一层），深渊框 `halo_abyss_<期数>` 按前缀识别、期数从 id 读，以后每期不用发版；顺带把奖池里一直没接的「晨曦光环」接上。增量再过一轮 Opus 审查（ship）+ Mac E2E（装扮页出现头像框栏 → 测试号拿到 halo_abyss_1 → 戴上 → 「我的」头像外圈四色环 + 12 刻度 + 王棋；深渊页满星奖励文案显示服务端下发的框名）。齐了。
- **cc-Yxi_Entertainment · 云曦小管家（形象 + 页面 + 入口）**（老板 2026-09-06 交办；PRD `Yxi_Entertainment/design/steward-prd.md`）：「我的」五宫格第一格「云曦」+ 侧栏「云曦」一行 + 提醒通知 extra `page=yunxi` 直达；页面 = 会动的 Q 版云曦（4 张 rembg 抠底 WebP，整图物理 + 姿势切换：进页挥手 / 待机捧云 / 办事举杖 / 22–6 点抱星睡；呼吸、看向手指、戳一下弹跳 + 台词、拖拽弹回；头上戴用户装备的头像框；可选表情图放了就自动接）+ 六张服务卡（备忘录含快捷提醒 / 重复 / 自定义时间 · 提醒列表 · 天气选城市 · 签到状态 · 深渊倒计时 · 未读邮件）。服务层 `app/yxi/yunxi/` 是 cc-Yxi_pilot 的（他自己记）。文件：`ui/YunxiPet.kt`（新）· `ui/YunxiScreen.kt`（新）· `MainActivity.kt`（Page.Yunxi + 路由 + 两处侧栏 + DrawerRow + 通知 extra）· `ui/SettingsScreen.kt`（五宫格）· `ui/En.kt`（+85，含 pilot 列的服务层文案与 23 个天气词）· `res/drawable-nodpi/yunxi_q_{wave,hold_cloud,wand,sleep}.webp`。**Opus 审查 ship（1 should-fix + 1 建议 + 3 nit 全改并复核）· 编译过 · i18n 归零 · Mac E2E 过**（五宫格 → 页面 → 戳她冒台词 → 记一条带 30 分钟提醒的备忘、提醒卡列出 → 搜「Shaoxing」选绍兴市拉到实时天气与三天预报 → 签到 / 深渊 / 邮件三卡真数据 → 侧栏入口进同一页）。齐了。
