...
- **cc-Bug_solverYxi · 「配置」和「我的」也能下拉刷新了**（老板 2026-09-07：「会话和主机都支持了，配置和我的也要」）：照 HostsScreen / SessionsScreen 那套 `PullToRefreshBox` 接的。配置 = 重读 `ConfigRemote.load`（**不清空旧的**，免得整页闪成转圈；空状态也能拉——那正是最想重试的时候）；我的 = 重拉 `/api/me` + 装扮归属，没登录就只转一下不空跑网络。（**编译 ✓**；⚠️ **设备上还没验**，模拟器被 Entertainment 占着，等它放开我拉一次看转圈和数据）
