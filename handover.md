...
第一百一十九轮：整合 entertainment D1 预算日期补丁（源为独立工作树 diff），缺日期不显示1970，按系统时区呈现。待回归。

第一百一十八轮：修复 UI Agent B15 地址栏 Enter 组合输入误触风险，待 Windows 回归。

第一百一十七轮：工作台首页增加清晰任务入口、连接状态和继续工作列表；待代理回归。

第一百一十六轮：按任务实例恢复主视图及右栏展开状态，应用内记忆；待 UI Agent 回归。

第一百一十五轮：工作台展示延后线路状态与取消/结果提示，待代理回归。

第一百一十四轮：延后线路纳入退出提示，写入期间阻止正常退出。hk13 默认 tmux 服务缺失，追加任务无法送达；报告仍未收尾。

第一百一十三轮：延后线路切换中枢等待/取消/结果流程，内存待办、终端空闲检查，非原子 turn 边界；待代理回归。

第一百一十二轮：Git 改动显示分支与提交，反馈引用实际读取时间。测试仍由四位 Agent 执行。

第一百一十一轮：整理工作台顶栏与路径，次要入口收进更多菜单；测试交给四位 Agent，当前暂无结论。

第一百一十轮：无 HMR 页面可选每 5 秒自动重载，操作/反馈期间暂缓，默认关闭。未验证。

第一百零九轮：Git 差异与修改意见可加入当前任务草稿，不自动发送。未验证。

第一百零八轮：Git diff 增删颜色、块位置层级和当前内容行数统计。未验证。

第一百零七轮：工作台 Git 改动只读页，切换暂存范围并打开文件右栏。未验证。

第一百零六轮：可选独立 Git worktree 启动，使用 HEAD、隔离目录，不自动合并或清理。未验证。

第一百零五轮：负责人汇总入口，引用成员和指派快照生成可编辑待发送任务。未验证。

第一百零四轮：协作负责人选择、持久化与上下文提示，共享核心保留 owners。未部署或验证。

第一百零三轮：协作组成员/指派/组规/投递控制分区，改进窄窗口与空状态。未验证。

第一百零二轮：组内新建 Claude/Codex 预填协作上下文，启动前注册成员；失败可能留离线成员。未验证。

第一百零一轮：新建 Claude/Codex 可填写启动提示词，直接作为独立参数传入；组上下文自动生成待接入。未验证。

第一百轮：Claude 协作上下文钩子独立安装/移除脚本与桌面导出，保留配置备份；Codex 注入待接入。未部署或验证。

第九十九轮：桌面配套 hub 脚本导出与安装说明，保留旧文件备份；不自动部署或注册钩子。未验证。

第九十八轮：回复链深度和重复正文限制，缺失/循环祖先拒绝追加；未部署或验证。

第九十七轮：桌面设置协作消息与时间预算，服务端返回计数/到期状态，界面显示到限。未部署或验证。

第九十六轮：hub 消息数/时间预算，登记锁内执行上限，暂停恢复保留预算；桌面设置待接入。未部署或验证。

第九十五轮：桌面协作组接入 hub 暂停/恢复/状态刷新，旧版显示未确认。未部署或验证。

第九十四轮：hub 组级 pause/resume/status 阻止后续投递，保留任务和记录；桌面控制待接入。未部署或验证。

第九十三轮：协作事件绑定双端 tmux 实例，回复和固定 ID 重试拒绝同名替代任务；投递定位 pane。未部署或验证。

第九十二轮：群发批次快照和成员固定消息 ID，支持显式 UUID 重试去重。未部署或验证。

第九十一轮：say/reply 固定 UUID 重试去重，未知投递拒绝重发；依赖保留日志，群发去重待补齐。未部署或验证。

第九十轮：协作事件按 ID 合并为消息卡片，支持原消息及直接回复搜索。未运行验证。

第八十九轮：yxi-hub reply 按消息 ID 回复原发送者，记录 replyTo 并核对同组关系；未部署、未发送、未验证。

第八十八轮：yxi-hub 消息 ID 与投递事件日志，桌面可切换事件源；未部署，未向 Agent 发消息，未验证。

第八十七轮：指派持久化组/主机/来源关联，协作组可查看本机指派状态和跳转目标。未运行验证。

第八十六轮：成员指派进入目标任务持久待发送队列，保留来源和唯一 ID，不自动投递。未运行验证。

第八十五轮：协作日志只读预览、搜索和刷新，展示当前服务器 hub.log 原文；不冒充处理回执。未运行验证。

第八十四轮：移除协作组入口与确认，保留成员任务和文件，不停止 Agent。未运行验证。

第八十三轮：协作组新建、成员与组规编辑接入服务器文件，保存不广播；保留其他组并提示已发现的并发修改。未运行验证。

第八十二轮：桌面协作组视图读取服务器组规与成员，支持打开成员任务；消息与自动协作仍待接入。未运行验证。

第八十一轮：插件说明和用户要求可加入同服务器当前任务草稿，绑定弹窗打开时的任务；不自动发送。未运行验证。

第八十轮：插件安装卡片增加服务器 README 与描述预览，显示主机、范围、版本和路径；按需只读。未运行验证。
