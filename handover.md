...
**1.1.19 之后攒的**：
- **cc-Yxi · 音游对齐试验台的漏项**（老板 09-07：「有很多昨天说的你没做」）：左下角英文名 Yxi Dancing Beat（曲名挪右下跟难度一起）· 爆点默认 1.0→1.1 · 旧谱下落 1.6→1.55 · 判定线慢摆 + 上下呼吸（试验台「谱面摆动 1.7」整段没搬）· 甩动回正改成试验台的弹簧（回正 2.7、cos 9t）· 画和触摸共用 `linePose()`。**老板 09-07 二次点名**「竖校准线 / 线翻转移动立竖 / 无线时刻都没做」→ 竖线加粗抽长（0.42s）、编舞全部重写成看得见的场景（`design/music/choreo.py` + `rechoreo_all.py`，16 张谱只改 lines）、立竖时轨道压进屏高、无线时刻 35%/下且只认最高档。**碎裂音效接入**（四种一块一种：tick 原声 · swipe 光尘 · trace 风铃散 · slide 樱瓣）· **12 条轨 + 音符加长 30% + 不画轨间线**（`Rhythm.LANES`，16 张谱经 `lanes.py` 铺开，units 不变）· **手指判定放宽到相邻轨** · **「▶ 演示」自动完美模式**（不上报）· **swipe 判定改成位移窗**（按住再划 / 不抬手连扫都算，模拟器 adb 三种划法全中，logcat 标签 YxiRhythm 可看每个判定）。规格 §1 / §5 / §7 / 坑 11 / #299。**过了**：opus 审查（无阻塞，四项逐个核过）· Mac 模拟器进局截图（左下英文名、右下曲名·难度、线在动、音符垂直于线）。
- **cc-Yxi_Entertainment · 微曦接进客户端**（2a2a5a5）：老板定的重复返还新规则（蓝 5 / 紫 7 / 金 10 微曦，角色六命后 50，10 微曦 = 1 曦光、满 10 自动进曦光），服务端早已上线，这是客户端那半。余额行显示零头 · 结算页折算文案**带换算**（「折 5 微曦（半抽）」——数字变大但价值变小，只写数字会被当成赚了）· 六命前的重复角色显示「已有 · 命座 +1」· **历史页按 kind 分开**（库里 tickets 70 条 / micro 18 条并存，一律写「折曦光」的话 5 微曦会显示成 5 曦光，差 10 倍）。**过了**：opus 子代理审查（四条全改，最重的一条是服务端 replay 分支漏字段、客户端会把零头抹成 0——logto 已修并加了字段集合断言，客户端也用 −1 哨兵兜了一手）· Mac 模拟器真接口 E2E（四种折算文案 / 零头更新 4→9 / 跨进位点 8+4→曦光+1 剩 2）· i18n 归零。
- **cc-Yxi_pilot · 长按「对话」翻自己发过的话**（老板 2026-09-07：「长按对话会展开一个画中画可以往前翻之前我们发出的内容，按会话计算」）：
  长按模式条上的「对话」→ 浮层列出**这个会话里自己发出去的每一条**（新的在上），点一条填回输入框、长按复制。
  `ui/SentHistory.kt`（新）· `ui/Workspace.kt`（ModeSwitcher 加 `onLongChat`，长按只给「对话」）· `ui/ChatScreen.kt`（`openSentTick` + `onSentShown`）· En +6。
  ⚠️ **数据不另存一份**：直接从转录里滤 `UserText` / `Queued` —— 「按会话计算」因此天然成立，也不会有「本地历史和服务器对话对不上」的第二个真相源。
  审查 fix-first 两条已修：① **切模式回来面板自己弹出来**（本想用计数省掉复位回调，但 ChatScreen 切模式时是**重建**的：`sentOpen` 复位而 tick 还非零 → `LaunchedEffect` 首次组合就触发，且每次切模式都弹）→ 加 `onSentShown` 用完清零；
  ② 带附件的消息显示成一串路径、填回输入框也带标记 → 用 `Attachments.parseRefs` 摘正文，附件只记个数。
  （**opus 审查 ✓ · 编译 ✓ · i18n 归零 · Mac E2E ✓**：真会话 14 条历史、点一条填回输入框、切终端再切回**不再自己弹**）

- **cc-Bug_solverYxi · 语音输入插到光标处 + 说满之后能看见新字**（老板 2026-09-07 报的两条，同一个根因）：草稿状态从 `String` 改成 `TextFieldValue` —— String 里没有光标，所以语音只能往尾巴上追加，而输入框超过 7 行之后滚到哪是跟着光标走的，光标不动新字就在视口外面。三条语音路统一走 `insertSpoken`。详见 TROUBLESHOOTING #298。（**编译 ✓ · 用例 `ui/SpokenInsertTest` 5 条**；⚠️ **设备上还没跑**，写的时候模拟器被 1.1.20 全套占着 —— 发版前那次全套会带上它）
