...
**1.1.19 之后攒的**：
- **cc-Yxi · 音游对齐试验台的漏项**（老板 09-07：「有很多昨天说的你没做」）：左下角英文名 Yxi Dancing Beat（曲名挪右下跟难度一起）· 爆点默认 1.0→1.1 · 旧谱下落 1.6→1.55 · 判定线慢摆 + 上下呼吸（试验台「谱面摆动 1.7」整段没搬）· 甩动回正改成试验台的弹簧（回正 2.7、cos 9t）· 画和触摸共用 `linePose()`。规格 §5 / §7 / 坑 11。**过了**：opus 审查（无阻塞，四项逐个核过）· Mac 模拟器进局截图（左下英文名、右下曲名·难度、线在动、音符垂直于线）。
