...
- **cc-logto_yxi · 上传第三轮修复(1fe895d)**:老板 2026-09-06「一次传很多图片 / 传视频还是失败」。真根因是**永远转圈不是报错** —— jsch 在无超时的管道流上等 SFTP 应答,而它自己的 keepalive 此时也发不出去(线程栈实证,TROUBLESHOOTING #277)。修:60 秒没进度就关通道/断连接、下次断点续传;重试抽成 `agent/Uploader.kt`,分享面板也走它。**已过 opus 审查 + Mac mini 实测**(不靠限速的确定性故障注入:等重连 ✓、传到一半掐断后续传且大小一字不差 ✓)。⚠️ 还差一半:`ChatScreen.startUpload` 换成调用 Uploader —— 等 Entertainment 的气泡改动提交后我落,落完再喊一声。

- **cc-Yxi · 侧边栏标出走内网的主机**（老板 2026-09-06）：抽屉里主机行加「内网」药丸，只按**这台手机怎么连它**判（`Host.viaTailscale` = 勾了内网且填了 100.x）。⚠️ 不是「这台主机有没有内网设备」——hk13 底下挂着一堆内网机器，但手机是走公网连它的，**不标**；tailscale测试 是手机走内网连的，**标**。药丸底色用 `tertiaryContainer` 不用 `secondaryContainer`：选中行的底色就是后者，同色会糊成一片。**编译过 · i18n 归零（「内网」En 里已有）· Mac 模拟器实测**（三台主机对照：Hyw / hk13 无标记、tailscale测试 有；选中态下药丸依然清晰）。审查还没做。
- **cc-Yxi · MCP 认证：回填没反应 + 让 Yxi 能接回调 + 按钮文案**（老板 2026-09-06 截图报的）：① 「交上去」点完输入框和按钮一起消失、上面还写着「等浏览器那边授权…」，**完全看不出交没交上** —— 加中性反馈 `Flow.note`（跟红色的 `hint` 分开），并且先 peek 一下服务器那头的会话还在不在，没了就直说「流程已结束，关掉重来」（会话没了时 `send-keys` 是静默空操作，用户会一直等到 10 分钟超时）。② 有些浏览器（荣耀自带 / 应用内 WebView）不自己跳 localhost，而是弹「使用以下方式打开」让你挑 App，选单里没有 Yxi → 只能手抄地址回填。加 manifest intent-filter（`http` + `localhost` + 路径钉死 `/callback`，不写端口 = 任意端口），MainActivity 收到就塞进 `Connect.pendingRedirect`，正开着的流程在「等授权」那步取走 = 替用户按了「交上去」；`Flow.init` 里清掉陈旧回调，免得没开框时收到的地址被下一个流程吃掉。③ 文案：「先关掉」→「关掉」、「去浏览器授权」→「认证」、「先关掉（后台继续装）」→「关掉（后台继续装）」。**编译过 · i18n 归零 · Mac 模拟器验过 intent-filter 确实注册上**（`cmd package query-activities` 里出现 app.yxi.MainActivity；打这个 URL 进 App 不崩、进程正常）。审查还没做；「认证 / 关掉」两个按钮在真 OAuth 流程里的样子还没截图（要一个活的 MCP 服务才走得到）。
